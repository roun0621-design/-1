...
async function loadRosterModalData(eventId) {
    ...
    try {
        ...
        const _rankTag = e => { const r = _pbRank.get(e.event_entry_id); if (!r) return ''; const isFinalR = evt.round_type === 'final' || heats.length === 1; const n = isFinalR ? r.all : r.heat; if (!n) return ''; const tot = isFinalR ? r.n_all : r.n_heat; const good = isFinalR ? n <= 3 : (_qHeat ? n <= _qHeat : n <= 3); return `<span class="pb-rank${good ? ' good' : ''}" title="PB 기준 ${isFinalR ? '전체' : '조'} 순번 (PB 있는 ${tot}명 중)">${n}/${tot}</span>`; };
