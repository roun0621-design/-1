...
function getFilteredEvents(cat) {
    ...
    return state.events.filter(e => {
        ...
        if (cat === 'relay') return e.category === 'relay';
        if (cat === 'road') return e.category === 'road';
        return true;
    });
}
...
async function processBarcodeOrBib(input) {
    ...
    try {
        // Send raw input — server handles all barcode format normalization
        const res = await API.checkinBarcode(input, state.callroomEventId);
...
function openTrackQualification() {
    ...
    document.getElementById('track-qual-list').innerHTML = rows.map((r, i) => `
        <div class="qual-checkbox-item">
            ...
            <span class="qual-name">${r.name} #${bib(r.bib_number)}</span>
...
function renderFieldContent() {
    ...
    const sorted = isView
        ? [...rows].sort((a, b) => { if (a.rank != null && b.rank != null) return a.rank - b.rank; if (a.rank != null) return -1; if (b.rank != null) return 1; return +(a.bib_number||0) - +(b.bib_number||0); })
...
function openFieldModal(eid) {
    ...
    document.getElementById('modal-athlete-info').textContent = `${entry.name}  #${bib(entry.bib_number)}`;
...
function renderCombinedStandings(subDefs) {
    ...
    document.getElementById('combined-standings-tbody').innerHTML = rows.map(r => {
        ...
        return `<tr>
            <td><strong>${r.rank}</strong></td><td>${bib(r.bib_number)}</td><td style="text-align:left;">${r.name}</td>
            ${cells}
...
async function loadResultsData() {
    ...
    if (cat === 'track') {
        ...
        document.getElementById('results-tbody').innerHTML = rows.map(r =>
            `<tr><td>${r.rank}</td><td>${r.lane_number || '—'}</td><td>${bib(r.bib_number)}</td><td style="text-align:left;">${r.name}</td><td>${r.team || ''}</td><td style="font-family:var(--font-mono);font-weight:600;">${r.time_seconds != null ? formatTime(r.time_seconds) : '—'}</td></tr>`
        ).join('');
        ...
        document.getElementById('results-tbody').innerHTML = rows.map(r => {
            ...
            return `<tr><td>${r.rank}</td><td>${bib(r.bib_number)}</td><td style="text-align:left;">${r.name}</td><td>${r.team || ''}</td>${c}<td style="font-weight:700;font-family:var(--font-mono);">${r.best != null ? r.best.toFixed(2) : '—'}</td></tr>`;
        }).join('');
        ...
        document.getElementById('results-tbody').innerHTML = rows.map(r => {
            ...
            return `<tr><td>${r.rank}</td><td>${bib(r.bib_number)}</td><td style="text-align:left;">${r.name}</td><td>${r.team || ''}</td>${c}<td style="font-weight:700;">${r.best != null ? r.best.toFixed(2) + 'm' : '—'}</td></tr>`;
        }).join('');
        ...
        document.getElementById('results-tbody').innerHTML = rows.map(r => {
            ...
            return `<tr><td>${r.rank}</td><td>${bib(r.bib_number)}</td><td style="text-align:left;">${r.name}</td>${c}<td><strong>${r.total || '—'}</strong></td></tr>`;
        }).join('');
    } else {
