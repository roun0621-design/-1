...
- 유닛 상태가 `RUNNING`(경기 중)이면 기록이 하나도 없어도 종목을 `in_progress`(카드 LIVE·진행 중 배지)로 — 여자 10,000m 처럼 주최 측이 중간 기록을 주지 않는 종목은 결승선까지 기록 0 이라 LIVE 가 안 떴다. `START_LIST` 로 되돌아가면 `created`.
- 조 완료 판정(2026-09-26): 공식(OFFICIAL) 이거나 **트랙·계주·도로에서 출전 전원에게 기록·상태가 들어오면** 조가 끝난 것으로 봐 라운드를 완료(카드 '결과', 알림). 필드는 1차 시기 뒤에도 전원 기록이 있으니 공식만. 완료 뒤에도 결과 읽기는 계속돼 정정·공식 확정이 반영된다.
- 빈 조(2026-09-26): 공식 일정엔 있지만 안 열린 조(시작 시각이 지났는데 Scheduled·명단 0명)는 `heat_flags` 에 `empty` 로 표시해 끝난 조로 친다. 남자 200m 예선 6조·10종 세부종목 빈 2조 때문에 라운드가 영원히 LIVE 로 남던 것.
- 일정 자동 갱신(2026-09-26): 대회 기간엔 `runOnce` 가 30분마다 `setupStructure` 를 다시 돌려 조 시각·시간표를 공식 일정에 맞춘다(9/26 여자 200m 예선 11:30 → 19:55 처럼 시각이 바뀜). 시각이 바뀌면 옛 시간표 줄은 지운다(`timetable_removed`). 결과 읽기 창(−1h~+12h)도 새 시각 기준.
- 라운드 이름은 주최 측 표기(PhaseDesc)를 따른다(2026-09-26): 남자 400m 은 유닛 코드가 SFNL 인데 공식 표기는 'Round 1'(예선 → 결승 직결) → 예선으로. 조·기록 없이 엔트리만 남은 사라진 라운드는 출전을 다음 라운드로 옮기고 지운다(빈 옛 조 포함). 확인된 변경: 남 400m 예선 없음, 여 200m 준결승 없음, 여 4x400m·여 멀리뛰기 예선 없음.
