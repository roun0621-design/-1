...
태블릿 입력 없이 종이 카드만 쓴 종목은 카드 사진을 AI(ChatGPT/Claude)로 xlsx 전사한 뒤 관리자 페이지 "필드 기록카드 가져오기"에 드롭해서 시기별 기록·풍속·높이 O/X 를 저장할 수 있다.
저장 형식은 이 문서의 규칙과 동일하다 (파울 0, 패스 -1, 높이 PASS, 기록구분은 NULL 행). 양식·표기·검산 규칙은 `docs/FIELD_CARD_IMPORT.md` 참고.
