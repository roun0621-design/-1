...
async function generateFullRecordPdf(db, comp, gender) {
  ...
  // 문서 양식 (심판장·기록원·로고) — 통합 db API (PG 호환). 예전엔 SQLite 전용 prepare 로 없는 열(templates)을 읽어 항상 null 이었다
  async function getDocTemplate(compId) {
    try {
      ...
      return { ad_card: parse(row.ad_card), start_list: parse(row.start_list), result_sheet: parse(row.result_sheet), comprehensive: parse(row.comprehensive) };
    } catch (e) { return null; }
  }
