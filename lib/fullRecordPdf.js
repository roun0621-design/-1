  ...
  for (const ws of wb.worksheets) {
    ...
    for (let r = 1; r <= ws.rowCount; r++) {
      ...
      for (let c = 1; c <= colWidths.length; c++) {
        ...
        // Skip cells covered by merges — do NOT advance x
        // The merge-start cell already drew the full merged width
        ...
        let cellW = colPts[c - 1];
        let cellH = rowH;
        let xAdvance = cellW; // how much to advance x after drawing
        ...
        if (merge) {
          ...
          xAdvance = cellW; // advance by the entire merged width
