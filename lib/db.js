...
function createSqliteAdapter(options = {}) {
    ...
    // 우선순위: options.path > 환경변수 SQLITE_PATH > 기본 경로
    // (테스트에서는 SQLITE_PATH=/tmp/test.db 같은 임시 경로로 격리)
    const DB_PATH = options.path
        || process.env.SQLITE_PATH
        || path.join(__dirname, '..', 'db', 'competition.db');
...
// schema.pg.sql 기준 자동 검증 완료 (2026-05-17, 2026-05-20).
const TABLES_WITHOUT_ID = new Set([
    'system_config',   // PK: key
    'event_records',   // PK: event_id
    'doc_template',    // PK: competition_id
    'division_master', // PK: code (Phase A v4)
    'award_docx_template', // PK: scope_key (워드 상장 양식, 2026-09)
]);
