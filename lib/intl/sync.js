...
async function syncEntries(db, comp, opts = {}) {
    ...
    for (const [base, { event }] of firstRound) {
        ...
        for (const b of before) {
            ...
            const used = await db.get('SELECT (SELECT COUNT(*) FROM heat_entry WHERE event_entry_id=?) + (SELECT COUNT(*) FROM result WHERE event_entry_id=?) AS c', b.entry_id, b.entry_id);
...
async function syncResults(db, comp, opts = {}) {
    const source = opts.source || parseSource(comp);
    const fetchFn = opts.fetch || B.fetchJson;
    const heats = await db.all(`SELECT h.*, e.id AS ev_id, e.category, e.round_status, e.gender, e.parent_event_id FROM heat h JOIN event e ON e.id=h.event_id WHERE e.competition_id=? AND h.external_key IS NOT NULL ORDER BY h.scheduled_at`, comp.id);
    const now = Date.now();
...
async function applyUnitResults(db, comp, heat, parsed, opts = {}) {
    ...
    if (opts.onApplied) { try { await opts.onApplied({ event_id: heat.ev_id, heat_id: heat.id, written, completed, official: !!parsed.official }); } catch (e) {} }
    return written;
}
