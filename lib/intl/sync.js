...
async function applyUnitResults(db, comp, heat, parsed, opts = {}) {
    ...
    // 공식 결과: 조를 official 로 표시하고, 종목의 모든 조가 official 이면 라운드 완료(결과 버튼·알림). 라운드가 이미 완료면 다시 알리지 않는다
    let completed = false;
    if (written && parsed.official) {
        const st = await _heatFlags(db, comp.id);
        st[String(heat.id)] = 'official';
        await saveState(db, comp.id, { heat_flags: st });
        const heats = await db.all('SELECT id FROM heat WHERE event_id=?', heat.ev_id);
        if (heats.length && heats.every(h => st[String(h.id)] === 'official') && heat.round_status !== 'completed') {
