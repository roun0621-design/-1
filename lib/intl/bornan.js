...
function parseSchedule(units) {
    ...
    for (const u of units || []) {
        ...
        const unit = { key: u.Key, no: k.unitNo, time: u.DateTimeRaw, venue: u.VenueDescS || u.Venue, status: u.StatusDesc || u.Status, label: u.UnitDescA || u.PhaseDescS, phaseLabel: u.PhaseDesc || u.PhaseDescS || '' };
    ...
    for (const ev of events) {
        ...
        // 라운드 이름은 주최 측 표기(PhaseDesc)를 따른다: 남자 400m 은 유닛 코드가 SFNL 인데 공식 표기는 'Round 1'(예선 → 결승 직결). 코드만 보면 '준결승'으로 잘못 보인다
        for (const r of ev.rounds) {
            const lbl = String((r.units[0] && r.units[0].phaseLabel) || '');
            if (/round\s*1|heats?\b|preliminar|qualif/i.test(lbl) && !/semi/i.test(lbl)) r.round_type = 'preliminary';
            else if (/semi/i.test(lbl)) r.round_type = 'semifinal';
            else if (/^final/i.test(lbl.trim())) r.round_type = 'final';
        }
...
function parseResults(json) {
    ...
    visitRecords(json, 0);
    // 진출 규칙 문구 (Results.Extensions[Code=QRules]) — "First 3 in each heat (Q) and next 2 fastest (q) advance to Final"
    let qrules = '';
    try { const ex = json && json.Results && Array.isArray(json.Results.Extensions) ? json.Results.Extensions.find(x => x && x.Code === 'QRules') : null; qrules = ex && ex.Value ? String(ex.Value).trim() : ''; } catch (e) {}
    ...
    if (!rows) return { rows: [], note, wind, official, records, qrules, status: String((json && json.Info && (json.Info.Status || json.Info.StatusDesc)) || json.Status || '').trim() };   // 명단 0명(Scheduled)도 상태는 준다
    const out = rows.map(r => {
        ...
        return {
            ...
            // 필드 시기표(Splits): 높이 → {dist:'1.71', att:'XO'} · 거리 → {dist:'3'(시도), res:'13.45'|'X'|'-', wind}
            splits: (Array.isArray(r.Splits) ? r.Splits : []).filter(s => s && (s.Category == null || s.Category === 'MAIN')).map(s => ({
                dist: String(s.Distance == null ? '' : s.Distance).trim(), att: String(s.AttResult == null ? '' : s.AttResult).trim(), res: String(s.Result == null ? '' : s.Result).trim(),
                status: String(s.Status || ''), wind: (() => { const n = parseFloat(ext(s, 'Wind')); return isFinite(n) ? n : null; })() })),
            raw: r,
        };
    });
    // 유닛 상태 원문(START_LIST · SCHEDULED · RUNNING · UNOFFICIAL · OFFICIAL …) — 스타트 리스트 단계의 기록은 받지 않는다
    const status = String((json && json.Info && (json.Info.Status || json.Info.StatusDesc)) || json.Status || '').trim();
    return { rows: out, note, wind, official, records, qrules, status };
}
