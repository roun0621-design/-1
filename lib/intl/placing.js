...
async function spotStatus(db, compId, spot, events) {
    ...
    const evById = new Map(events.map(e => [Number(e.id), e]));
    ...
    for (const e of events) {
        ...
        const list = withRes.map(r => ({ entry_id: r.entry_id, name: r.name, name_alt: r.name_alt, is_team: /^RELAY_/.test(String(r.barcode || '')), heat_number: r.heat_number, lane: r.lane_number,
            ...
            status_code: r.status_code || null, qual: (String(r.remark || '').match(/\b(Q|q)\b/) || [null])[0] }));
        ...
        if (e.round_type === 'final') {
            ...
            const base = baseOf(e);
            const hasSemi = [...evById.values()].some(x => baseOf(x) === base && x.round_type === 'semifinal');
...
module.exports = { placesInHeats, spotStatus, qualRuleKo };
