...
// ────────────── 포맷 헬퍼 ──────────────
// 초 소수 자리수: 기본 2자리, 3번째 자리가 유효하면 3자리 (FAT 1/1000 보존, 화면 표기와 일치)
function _secDp(v) {
...
async function getTopResultsForEvent(db, evt) {
  ...
  if (isHeight) {
    ...
    for (const e of entries) {
      ...
      // 시기는 치렀으나(높이 시도 기록 있음) 성공 높이가 없으면 NM (기록없음)
      if (bestCleared === null && !status && myAttempts.length > 0) status = 'NM';
    ...
    athleteData.sort(PaceRanking.withStatusLast((a, b) => {
      ...
      // 같은 높이 → 수동 순위(순위결정전) 우선, 없으면 countback(실패수)
    ...
    // DNS 만 제외하고 NM/DNF/DQ 는 종합기록지에 표기 (기록 없어도 NM 으로 노출)
    const rankings = athleteData
      ...
      .map(a => ({
        name: a.name || '',
        team: a.team || '',
        record: a.bestCleared != null ? fmtFieldDist(a.bestCleared) : (a.status_code || 'NM'),
        wind: null,
      }));
  ...
  if (isDist) {
    ...
    const rankings = allEntries
      ...
      .map(a => ({
        name: a.name || '',
        team: a.team || '',
        // 유효 기록 있으면 미터, 없으면 상태코드(없으면 NM) — 기록 없어도 표기
        record: a.best != null ? fmtFieldDist(a.best) : (a.status_code || 'NM'),
  ...
  deduped.sort(PaceRanking.withStatusLast((a, b) => {
    if (a.best == null && b.best == null) return 0;
    if (a.best == null) return 1; if (b.best == null) return -1;
