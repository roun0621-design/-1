...
function normalizePhone(phone) {
    ...
    else if (p.startsWith('82') && p.length >= 12) p = '0' + p.slice(2);   // '82' 로 시작하는 국내 번호(예: 0820…)와 구분 — 국가번호 형태는 12자리 이상
    return p;
}
/** 발송 가능한 휴대폰 번호인가 — 01x 로 시작하는 10~11자리 (예전엔 검증 없이 어떤 값이든 발송 시도했다) */
...
async function sendOne(config, params) {
    ...
    // ── 시뮬레이션 모드 우선 판정 ─────────────────────────────
    // sim_mode=1 이거나 api_key 미설정이면 실제 발송 없이 DB 기록만.
    // (발신번호·수신번호 미설정이어도 테스트/미리보기가 가능하도록 시뮬레이션 처리)
    // 단, 수신번호가 없으면 'skipped'(건너뜀)으로 명확히 구분한다.
    if (config.sim_mode || !config.api_key) {
        if (!phone) {
            return {
                status: 'skipped',
                provider_msg_id: '',
                error_message: '수신번호 없음 (선수 연락처 미등록)',
                cost: 0,
                msg_type: msgType,
            };
        }
    ...
    // ── 실제 발송 모드 ───────────────────────────────────────
    if (!phone) return { status: 'failed', provider_msg_id: '', error_message: '수신번호 없음 (선수 연락처 미등록)', cost: 0, msg_type: msgType };
    if (!sender) return { status: 'failed', provider_msg_id: '', error_message: '발신번호 미설정', cost: 0, msg_type: msgType };

    // 알리고 실제 발송
    if (config.provider !== 'aligo') {
        return { status: 'failed', provider_msg_id: '', error_message: '지원하지 않는 provider: ' + config.provider, cost: 0, msg_type: msgType };
    }
