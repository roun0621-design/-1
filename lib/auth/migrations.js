...
async function runAuthMigrations(db) {
    ...
    try {
        ...
        // (2026-09) 부팅마다 legacy admin_pw 로 비밀번호를 덮어쓰던 재동기화 제거.
        //   계정 화면에서 바꾼 JWT 비밀번호가 다음 재시작 때 옛 값으로 되돌아가던 원인.
        //   JWT 브리지 도입으로 관리자 화면이 legacy 키 없이 동작하므로 두 자격증명을 분리한다.
        //   최초 1회 시드(위 INSERT)만 legacy 해시를 복사한다.
    } catch (e) {
