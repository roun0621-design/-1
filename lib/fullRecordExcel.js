...
const PaceRanking = require('../public/lib/ranking');   // 순위·상태코드 공용 규칙
...
// ---- Format helpers ----
// 초 소수 자리수: 기본 2자리, 3번째 자리가 유효(0이 아님)하면 3자리 (FAT 1/1000 입력 보존)
// 프론트 common.js _decPlaces 와 동일 규칙 → 화면과 기록지 표기 일치
function _secDp(v) {
  const r3 = Math.round(v * 1000) / 1000;
  const r2 = Math.round(v * 100) / 100;
  return Math.abs(r3 - r2) < 0.0001 ? 2 : 3;
}
function fmtTime(seconds) {
  ...
  if (seconds >= 3600) {
    ...
    return `${h}:${m < 10 ? '0' : ''}${m}:${r < 10 ? '0' : ''}${r.toFixed(_secDp(r))}`;
  }
  if (seconds >= 60) {
    const m = Math.floor(seconds / 60);
    const r = seconds - m * 60;
    return `${m}:${r < 10 ? '0' : ''}${r.toFixed(_secDp(r))}`;
  }
  return seconds.toFixed(_secDp(seconds));
}
...
// ── 신기록(NR/DR/CR) 표기용 ──
// 기준기록 문자열 → 숫자 (예: "12m41"/"12.41"→12.41, "2:15.00"→135, "1.95"→1.95)
function _xlParseRecNum(s) {
  if (s == null) return null;
  const t = String(s).trim();
  if (!t) return null;
  const mm = t.match(/^(\d+)m(\d+)$/);            // 1m95 / 12m41
...
// 최고기록(numBest)이 기준(recObj: {national,division,competition})을 깬 라벨 ("CR"/"NR DR CR")
function _xlBrokenLabels(numBest, direction, recObj, bestWind, isWindEvt) {
  if (numBest == null || !isFinite(numBest) || !direction || !recObj) return '';
  if (isWindEvt && bestWind != null && Number(bestWind) > 2.0) return ''; // 풍속 초과 = 참고기록(불인정)
  const out = [];
  for (const [key, label] of [['national', 'NR'], ['division', 'DR'], ['competition', 'CR']]) {
    const rec = recObj[key];
    if (!rec) continue;
    const oldVal = _xlParseRecNum(rec.record_value);
    if (oldVal == null) continue;
    if (direction === 'lower' && numBest < oldVal) out.push(label);
    else if (direction === 'higher' && numBest > oldVal) out.push(label);
  }
  return out.join(' ');
}
function fmtWind(w) {
...
// ---- 대회 데이터 프리로드 (2026-09) ----
//   예전엔 db.prepare()(SQLite 전용 동기 API)를 정렬 비교 함수·map 안에서까지 호출해 PostgreSQL 에서는 종합기록지가 아예 만들어지지 않았다.
//   대회 단위로 한 번에 읽어 두고(쿼리 9개) 아래 레이아웃 코드는 동기 조회(D.xxx)로 쓴다.
async function _preload(db, comp) {
  const by = (rows, key) => { const m = new Map(); for (const r of rows) { const k = r[key]; if (!m.has(k)) m.set(k, []); m.get(k).push(r); } return m; };
  const heats = await db.all('SELECT h.* FROM heat h JOIN event e ON e.id=h.event_id WHERE e.competition_id=? ORDER BY h.heat_number', comp.id);
  const entries = await db.all(`SELECT he.heat_id, he.lane_number, ee.id AS event_entry_id, ee.status, ee.athlete_id, a.name, a.bib_number, a.team
    FROM heat_entry he JOIN event_entry ee ON ee.id=he.event_entry_id JOIN athlete a ON a.id=ee.athlete_id JOIN heat h ON h.id=he.heat_id JOIN event e ON e.id=h.event_id
    WHERE e.competition_id=? ORDER BY he.heat_id, he.id`, comp.id);
  const results = await db.all(`SELECT r.* FROM result r JOIN heat h ON h.id=r.heat_id JOIN event e ON e.id=h.event_id WHERE e.competition_id=? ORDER BY r.event_entry_id, r.attempt_number`, comp.id);
  const attempts = await db.all(`SELECT ha.* FROM height_attempt ha JOIN heat h ON h.id=ha.heat_id JOIN event e ON e.id=h.event_id WHERE e.competition_id=? ORDER BY ha.bar_height, ha.event_entry_id, ha.attempt_number`, comp.id);
  const scores = await db.all(`SELECT cs.* FROM combined_score cs JOIN event_entry ee ON ee.id=cs.event_entry_id JOIN event e ON e.id=ee.event_id WHERE e.competition_id=? ORDER BY cs.sub_event_order`, comp.id);
  const members = await db.all(`SELECT rm.event_entry_id, a.name, rm.leg_order, a.bib_number FROM relay_member rm JOIN athlete a ON a.id=rm.athlete_id JOIN event_entry ee ON ee.id=rm.event_entry_id JOIN event e ON e.id=ee.event_id
    WHERE e.competition_id=? ORDER BY rm.event_entry_id, rm.leg_order`, comp.id);
  let tt = [], evRecs = [], quals = [];
  try { tt = await db.all('SELECT event_id, day FROM timetable WHERE competition_id=? AND event_id IS NOT NULL', comp.id); } catch (e) {}
  try { evRecs = await db.all('SELECT er.event_id, er.records FROM event_records er JOIN event e ON e.id=er.event_id WHERE e.competition_id=?', comp.id); } catch (e) {}
  try { quals = await db.all('SELECT qs.event_id, qs.event_entry_id, qs.qualification_type FROM qualification_selection qs JOIN event e ON e.id=qs.event_id WHERE e.competition_id=? AND qs.selected=1', comp.id); } catch (e) {}
  const heatsBy = by(heats, 'event_id'), entriesBy = by(entries, 'heat_id'), resultsBy = by(results, 'heat_id'), attemptsBy = by(attempts, 'heat_id'), scoresBy = by(scores, 'event_entry_id'), membersBy = by(members, 'event_entry_id'), qualsBy = by(quals, 'event_id');
  const ttBy = new Map(tt.map(r => [r.event_id, r])), evRecBy = new Map(evRecs.map(r => [r.event_id, r]));
  const bibInt = b => { const n = parseInt(String(b || '').replace(/\D/g, ''), 10); return Number.isFinite(n) ? n : 0; };
  return {
    heatsOf: id => heatsBy.get(id) || [],
    entriesOf: id => entriesBy.get(id) || [],
    resultsOf: id => resultsBy.get(id) || [],
    attemptsOf: id => attemptsBy.get(id) || [],
    scoresOf: id => scoresBy.get(id) || [],
    membersOf: id => (membersBy.get(id) || []).slice().sort((a, b) => ((a.leg_order || 0) - (b.leg_order || 0)) || (bibInt(a.bib_number) - bibInt(b.bib_number))).map(m => m.name),
    qualsOf: id => qualsBy.get(id) || [],
    ttDay: id => ttBy.get(id) || null,
    eventRecordsRow: id => evRecBy.get(id) || null,
    minHeat: id => { const hs = heatsBy.get(id) || []; return { mn: hs.length ? Math.min(...hs.map(h => h.heat_number)) : null }; },
  };
}
...
async function generateFullRecordExcel(db, comp, gender, getDocTemplate) {
  ...
  const eventRecords = {};
  try {
    // CR(대회기록)은 '이 대회가 연결된 시리즈'의 기록만 사용 (다른 시리즈 CR 혼입 방지)
    const _compSeriesId = (comp && comp.series_id != null) ? comp.series_id : null;
    const recRows = await db.all('SELECT * FROM event_record WHERE gender=?', gender);
    for (const r of recRows) {
      if (r.record_type === 'national') { if (r.series_id != null || r.division_code != null) continue; }      // NR = 전국(시리즈/부 없음)
      else if (r.record_type === 'division') { if (r.series_id != null) continue; }                            // DR 은 시리즈 기록 아님
      else if (r.record_type === 'competition') { if (_compSeriesId == null || r.series_id !== _compSeriesId) continue; } // CR = 이 대회 시리즈만
      // 종목명 정규화 키로 저장 — 매트릭스/상세시트가 정규화된 evtName(예: 3000mSC)으로 조회하므로 키 일치 필요
      const _evKey = normalizeEventName(r.event_name);
      if (!eventRecords[_evKey]) eventRecords[_evKey] = {};
      eventRecords[_evKey][r.record_type] = r;
    }
  } catch(e) { /* event_record table might not exist */ }

  const D = await _preload(db, comp);
  ...
  const tpl = await getDocTemplate(comp.id);      // server.js 의 getDocTemplate 은 async — 예전엔 await 없이 Promise 를 받아 심판장·기록원이 항상 빈칸이었다
  ...
  function getEventRecordsForEvent(eventId) {
    try {
      const row = D.eventRecordsRow(eventId);
  ...
  function getEventRankings(evt) {
    ...
    const heats = D.heatsOf(evt.id);
    if (heats.length === 0) return [];

    if (isCombined) {
      const heat = heats[0];
      const entries = D.entriesOf(heat.id);
      return entries.map(e => {
        const scores = D.scoresOf(e.event_entry_id);
        let totalPoints = scores.reduce((s, sc) => s + (sc.wa_points || 0), 0);
        const status = D.resultsOf(heat.id).find(r => r.event_entry_id === e.event_entry_id && ['DNF', 'DNS', 'DQ'].includes(r.status_code)) || null;
      ...
      }).sort(PaceRanking.withStatusLast((a, b) => parseInt(b.record) - parseInt(a.record)));
    } else if (isFieldHeight) {
      const heat = heats[0];
      const entries = D.entriesOf(heat.id);
      const allAttempts = D.attemptsOf(heat.id);
      return entries.map(e => {
        ...
        const results = D.resultsOf(heat.id).filter(r => r.event_entry_id === e.event_entry_id);
      ...
      for (const heat of heats) {
        const entries = D.entriesOf(heat.id);
        const results = D.resultsOf(heat.id);
        const resMap = {};
        ...
        for (const e of entries) {
          ...
          allEntries.push({ ...e, best, bestWind, status_code: sc, record: sc || fmtDist(best), attempts, sortedValid: recs.filter(r => r.distance_meters != null && r.distance_meters > 0 && r.status_code !== 'X' && r.status_code !== 'FOUL').map(r => r.distance_meters).sort((x, y) => y - x) });
        }
      }
      return allEntries.sort(PaceRanking.withStatusLast((a, b) => {
      ...
      for (const heat of heats) {
        ...
        for (const e of entries) {
          ...
          if (isRelay) {
            entry.members = D.membersOf(e.event_entry_id);
          }
  ...
  // ===== SHEET 1: 종합기록 =====
  {
    ...
    for (const evtName of SHEET_ORDER) {
      ...
      // 신기록(NR/DR/CR) 판정 — 각 위(rank)별, 기준은 이 대회 시리즈 기록(eventRecords)
      const _recBaseObj = eventRecords[evtName] || null;
      const _recDir = (finalEvt.category === 'field_distance' || finalEvt.category === 'field_height') ? 'higher'
                    : (finalEvt.category === 'track' || finalEvt.category === 'road' || finalEvt.category === 'relay') ? 'lower' : null;
      const recLabels = [];
      for (let i = 0; i < 8; i++) {
        const r = filtered[i];
        if (!r || !_recBaseObj || !_recDir) { recLabels.push(''); continue; }
        const numBest = (r.bestCleared != null) ? r.bestCleared : (r.best != null ? r.best : null);
        recLabels.push(_xlBrokenLabels(numBest, _recDir, _recBaseObj, r.bestWind, hasWind));
      }
      const hasAnyRecord = recLabels.some(Boolean);
      ...
      ws.getRow(row).height = (needsSecondRow || hasAnyRecord) ? 13.5 : 27;
      const dataRow = row;
      row++;

      // Row 2: 풍속 / 릴레이 정보 행 (기존)
      if (needsSecondRow) {
        setCell(ws, row, 1, '', { border: borders.all });
      ...
      // Row 3: 신기록 라벨(NR/DR/CR) — 기록 칸 맨 아래 줄. 신기록 있는 종목만 추가.
      if (hasAnyRecord) {
        setCell(ws, row, 1, '', { border: borders.all });
        setCell(ws, row, 2, '', { border: borders.all });
        for (let i = 0; i < 8; i++) {
          ...
          setCell(ws, row, baseCol, '', { border: borders.all });
          setCell(ws, row, baseCol + 1, '', { border: borders.all });
          setCell(ws, row, baseCol + 2, recLabels[i] || '', {
            font: { name: '맑은 고딕', size: 8, bold: true, color: { argb: 'FF27AE60' } }, alignment: alignC, border: borders.all
          });
        }
      ...
      // 블록 세로 병합: A·종목(B)·성명·소속 칸을 블록 전체 높이로 (기록/풍속/라벨 줄 제외)
      const lastRow = row - 1;
      if (lastRow > dataRow) {
        for (let i = 0; i < 8; i++) {
          const baseCol = 3 + i * 3;
          try { ws.mergeCells(dataRow, baseCol, lastRow, baseCol); } catch(e) {}       // 성명
          try { ws.mergeCells(dataRow, baseCol + 1, lastRow, baseCol + 1); } catch(e) {} // 소속
          // 릴레이는 기록값 세로 병합 (라벨 행 없을 때만)
          if (isRelay && !hasWind && !isJumpEvt && !hasAnyRecord) {
            try { ws.mergeCells(dataRow, baseCol + 2, lastRow, baseCol + 2); } catch(e) {}
          }
        }
        try { ws.mergeCells(dataRow, 1, lastRow, 1); } catch(e) {}
        try { ws.mergeCells(dataRow, 2, lastRow, 2); } catch(e) {}
      }
    }
  }
  ...
  try {
    const recordBreaks = await db.all(`
      SELECT rbl.*,
  ...
  for (const evtName of SHEET_ORDER) {
    ...
    const prelimEvts = sortedEvts.filter(e => e.round_type === 'preliminary').sort((a, b) => {
      const ha = D.minHeat(a.id);
      const hb = D.minHeat(b.id);
    ...
    for (const evt of orderedEvts) {
      ...
      let qualMap = {};
      if (!isFinal) {
        const quals = D.qualsOf(evt.id);
        for (const q of quals) { qualMap[q.event_entry_id] = q.qualification_type || 'Q'; }
      }

      for (const heat of heats) {
        const heatEntries = D.entriesOf(heat.id);
        const heatResults = D.resultsOf(heat.id);
        const resMap = {};
        ...
        let rank = 1;
        for (const e of entries) {
          const isSpecial = PaceRanking.isStatus(e.status_code);
          ...
          // DNF/DQ → 기록란 공백, 비고란에만 표시
          const isDnfDq = isSpecial;   // 상태코드는 기록란 공백, 비고란에만
