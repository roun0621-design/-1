...
async function generateFullRecordExcel(db, comp, gender, getDocTemplate) {
  ...
  try {
    const _ck = (comp && comp.series_id != null) ? comp.series_id : null;
    console.log(`[fullRecordExcel] gender=${gender} series_id=${_ck} eventRecords keys=${JSON.stringify(Object.keys(eventRecords))}`);
  } catch(e) {}
  ...
  // ===== SHEET 1: 종합기록 =====
  {
    ...
    for (const evtName of SHEET_ORDER) {
      ...
      try {
        if (_recBaseObj || filtered[0]) {
          const b0 = filtered[0] ? (filtered[0].bestCleared != null ? filtered[0].bestCleared : filtered[0].best) : null;
          console.log(`[fullRecordExcel] evt=${evtName} cat=${finalEvt.category} dir=${_recDir} base=${_recBaseObj ? JSON.stringify(Object.keys(_recBaseObj)) : 'NONE'} top=${b0} labels=${JSON.stringify(recLabels.filter(Boolean))}`);
        }
      } catch(e) {}
