...
function computeHeight(g) {
    ...
    for (const r of g.rows) {
        // 순위 규칙은 서버·브라우저 공용 모듈 (WA TR 26.2·26.8)
        const hd = {}; for (const h of heights) { const marks = r.marks[hk(h)]; if (marks) hd[h] = marks; }
        const st = PaceRanking.heightStats(hd, heights);
        r.computed.best = st.best;
        r.computed.failsAtBest = st.failsAtBest;
        r.computed.totalFails = st.totalFails;
        r.computed.eliminated = st.eliminated;
        r.computed.rank = null;
    }
    const ranked = g.rows.filter(r => r.computed.best != null && !r.status.code).sort((a, b) => {
        if (b.computed.best !== a.computed.best) return b.computed.best - a.computed.best;
