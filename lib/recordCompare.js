...
// ─── 공식 기록 환산 (WA TR 19.24) ───────────────────────────
//   트랙·릴레이: 1/100초 단위로 올림 (10.213 → 10.22, 10.210 → 10.21) / 도로: 1초 단위로 올림
function officialTime(seconds, category) {
    if (typeof seconds !== 'number' || !isFinite(seconds)) return seconds;
    const unit = category === 'road' ? 1 : 100;
    return Math.ceil(Math.round(seconds * 1000) / 1000 * unit - 1e-9) / unit;
}

// ─── 판정에 쓸 풍속 ──────────────────────────────────────────
//   필드(멀리/세단)는 시기별 result.wind, 트랙(100/200/허들)은 조 단위 heat.wind("1.8 m/s" 문자열)로 저장된다.
//   (2026-09) 예전엔 result.wind 만 봐서 트랙의 추풍 기록이 신기록으로 감지됐다.
function effectiveWind(result, heat) {
    if (result && typeof result.wind === 'number' && isFinite(result.wind)) return result.wind;
    if (heat && heat.wind != null && heat.wind !== '') {
        const w = parseFloat(String(heat.wind));
        if (isFinite(w)) return w;
    }
    return null;
}

// ─── 종목명 정규화 — lib/eventName.js 로 이동 (출력 동일) ───
const { recordKey: normalizeEventName } = require('./eventName');
const { divisionCodeFor } = require('./division');

/**
 * 종목의 부 라벨 → 부 마스터 코드 (DR 비교용). 2026-09 Phase 7-②
 *   전에는 event.division 이 코드(M_MID)와 글자 그대로 같을 때만 DR 을 봤는데, 관리자 화면·조편성 업로드는
 *   '중등부'·'고등부' 같은 라벨을 저장하므로 부별 기록이 한 번도 감지되지 않았다. 라벨·학년·성별로 코드를 찾는다.
 */
async function resolveDivisionCode(db, event) {
    if (!event || !event.division) return null;
    const masters = await db.all('SELECT code, label_ko, gender, school_level, grade FROM division_master WHERE active=1');
    return divisionCodeFor(event.division, event.gender, masters);
}
...
function parseRecordValue(v) {
    ...
    if (!s) return null;
    // ⚠️ 콤마 천단위 구분자 제거 (combined 종목 점수에서 "5,535" 같은 표기 대응)
    //     parseFloat("5,535") 는 "5" 까지만 파싱해서 신기록 비교가 완전히 망가짐.
    //     한글 콤마(，) 도 함께 제거.
    s = s.replace(/[,，]/g, '');
    if (s.includes(':')) {
...
// 비현실적 비교 차단 (안전망):
//  - parseRecordValue 가 "5,535" → 5 같은 파싱 버그를 갖던 시절,
//    higher 종목에서 newVal(3754) > oldVal(5) 로 잘못 인정되는 사고가 발생함.
//  - 콤마 제거는 이미 적용되었지만, 외부 입력값에서 사람이 "5535" / "5,535" / "5 535"
//    이외의 비정상 표기를 넣었을 경우를 대비해 한 번 더 가드.
//  - 규칙: 두 값 모두 양수이고 한 쪽이 다른 쪽의 10배 이상 차이가 나면 의심 케이스 →
//          neighbor record(콤마 누락) 가능성으로 보고 신기록 인정 거부.
const SUSPICIOUS_RATIO = 10;
function isSuspiciousDiff(newVal, oldVal) {
    if (!isFinite(newVal) || !isFinite(oldVal)) return false;
    if (newVal <= 0 || oldVal <= 0) return false;
    const ratio = Math.max(newVal, oldVal) / Math.min(newVal, oldVal);
    return ratio >= SUSPICIOUS_RATIO;
}
...
function isBetter(newVal, oldVal, direction) {
    ...
    // 안전망: higher 종목에서 10배 이상 차이가 나면 파싱 오류 가능성 → 거부
    if (direction === 'higher' && isSuspiciousDiff(newVal, oldVal)) {
        console.warn(`[recordCompare] 의심 비교 (higher) — new=${newVal} vs old=${oldVal}: ` +
            `10배 이상 차이로 파싱 오류 가능성 → 신기록 인정 거부`);
        return false;
    }
...
/** 타이기록(CT·DT·KT, Phase 7-④): 공식 기록(1/100초·cm·점)이 기존 기록과 같으면 */
function isTie(newVal, oldVal, direction) {
    if (newVal == null || oldVal == null || !isFinite(newVal) || !isFinite(oldVal)) return false;
    if (direction === 'higher' && isSuspiciousDiff(newVal, oldVal)) return false;
...
async function compareAndLog(db, ctx, record_type, division_code, series_id) {
    ...
    // (2026-09) 비교할 기존 기록이 없으면 감지하지 않는다.
    //   예전엔 "기록 없음 → 무조건 신기록" 이라, 기록표에 없는 종목·시리즈 미등록 종목은 종목 1위가 전부
    //   '한국신기록/대회신기록' 승인 대기로 올라갔다. 화면 배지(detectBrokenRecordsClient)는 원래 기준 없으면 표시 안 함 → 서버도 맞춤.
    //   신설 종목의 첫 기록은 관리자 → 기록 관리에서 직접 등록한다.
    if (!existing || existingNum == null) return null;
    const tie = !isBetter(newValueNum, existingNum, direction) && isTie(newValueNum, existingNum, direction);
    if (!tie && !isBetter(newValueNum, existingNum, direction)) return null;

    // newValue 문자열 표현
    const newValueStr = formatValueForDisplay(newValueNum, direction, event.category);

    // 같은 종목/타입/부/시리즈에서 이 값보다 '나쁜' 다른 선수의 pending 은 제거 → 최고 1명만 남김.
    // (느린→빠른 순으로 입력돼 이전 리더가 먼저 pending 됐던 경우를 정리)
    {
        const dcCl = division_code == null ? 'division_code IS NULL' : 'division_code = ?';
        const siCl = series_id == null ? 'series_id IS NULL' : 'series_id = ?';
        const worseCmp = direction === 'higher' ? 'new_value_num < ?' : 'new_value_num > ?';
        const params = [event.id, record_type];
        if (division_code != null) params.push(division_code);
        if (series_id != null) params.push(series_id);
        params.push(result.event_entry_id, newValueNum);
        await db.run(
            `DELETE FROM record_breaking_log
             WHERE event_id=? AND record_type=? AND ${dcCl} AND ${siCl}
               AND status='pending' AND event_entry_id<>? AND ${worseCmp}`,
            ...params
        );
    }
    ...
    if (existingPending) {
        ...
        return { id: existingPending.id, updated: true, record_type, division_code, series_id, is_tie: tie };
    }
    ...
    const info = await db.run(
        `INSERT INTO record_breaking_log
         (competition_id, event_id, event_entry_id, record_type, event_name, gender,
          ...
          athlete_name, athlete_team, bib_number, wind, is_tie, status)
         VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,'pending')`,
    ...
    return { id: info.lastInsertRowid, created: true, record_type, division_code, series_id, is_tie: tie };
}
...
// ============================================================
// 종목(모든 heat) 안에 newValueNum 보다 '엄격히 더 좋은' 다른 선수 결과가 있는가?
// → 있으면 이 선수는 신기록 보유자가 아니므로 감지/팝업 대상에서 제외 (그중 최고 1명만).
async function _hasBetterResultInEvent(db, eventId, excludeEntryId, direction, newValueNum) {
    try {
        const heats = await db.all('SELECT id FROM heat WHERE event_id=?', eventId);
        if (!heats || !heats.length) return false;
        const hIds = heats.map(h => h.id);
        const ph = hIds.map(() => '?').join(',');
        if (direction === 'higher') {
            const row = await db.get(
                `SELECT MAX(distance_meters) AS v FROM result
                 WHERE heat_id IN (${ph}) AND event_entry_id<>?
                   AND distance_meters IS NOT NULL AND distance_meters>0
                   AND (status_code IS NULL OR status_code='')`,
                ...hIds, excludeEntryId);
            return !!(row && row.v != null && row.v > newValueNum + 1e-9);
        }
        const row = await db.get(
            `SELECT MIN(time_seconds) AS v FROM result
             WHERE heat_id IN (${ph}) AND event_entry_id<>?
...
async function detectRecordBreaks(db, ctx) {
    try {
        ...
        // 공식 기록(WA TR 19.24): 1/1000초는 순위 판정용일 뿐, 공식 기록은 1/100초로 '올림'(도로 종목은 1초로 올림).
        //   10.213 은 공식 10.22 → 10.22 기록과 '동률'이지 경신이 아니다. 예전엔 입력된 1/1000 값 그대로 비교했다.
        if (direction === 'lower') newValueNum = officialTime(newValueNum, event.category);
        ...
        if (isWindAffectedEvent(normalizedName)) {
            const wind = effectiveWind(result, ctx.heat);
            windUsed = wind;
        ...
        // ─── 종목 내 최고기록자만 감지 (그중 최고 1명만 신기록 팝업/등록) ───
        // 종전 기록을 넘었더라도, 같은 종목에 더 좋은 결과가 이미 있으면 이 선수는
        // 신기록 보유자가 아니므로 감지하지 않는다. (비고 CR 배지는 클라이언트가 별도 표시)
        if (await _hasBetterResultInEvent(db, event.id, result.event_entry_id, direction, newValueNum)) {
            return { detected: [], skipped: 'not-event-best' };
        }

        // 로그에 남길 풍속도 조 풍속을 반영
        const enrichedCtx = { ...ctx, result: { ...result, wind: (windUsed != null ? windUsed : result.wind) }, newValueNum, direction, normalizedName };
        const detected = [];
        ...
        // 2) DR (division) — 종목의 부 라벨('중등부'·'중1학년부'·코드)이 부 마스터로 풀릴 때만
        const divCode = await resolveDivisionCode(db, event);
        if (divCode) {
            const drRes = await compareAndLog(db, enrichedCtx, 'division', divCode, null);
...
async function detectCombinedRecordBreaks(db, ctx) {
    try {
        ...
        // ─── 종합경기 풍속 규정 (WA TR 39 / 기록 공인): 풍속을 재는 세부종목(10종: 100m·멀리뛰기·110mH / 7종: 100mH·200m·멀리뛰기)의
        //     평균 풍속이 +2.0 m/s 를 넘으면 기록으로 공인되지 않는다 → 참고기록, 감지하지 않음.
        //     풍속이 하나라도 입력되지 않았으면 판단을 보류하고(막지 않음) 기록위원 승인 단계에 맡긴다.
        try {
            const subs = await db.all('SELECT id, name, category FROM event WHERE parent_event_id=?', parent_event.id);
            const winds = [];
            let missing = false;
            for (const se of subs) {
                const nm = normalizeEventName(String(se.name || '').replace(/^\[[^\]]+\]\s*/, ''));
                if (!isWindAffectedEvent(nm)) continue;
                const subEntry = await db.get('SELECT id FROM event_entry WHERE event_id=? AND athlete_id=?', se.id, athlete.id);
                if (!subEntry) { missing = true; continue; }
                if (String(se.category || '').startsWith('field')) {
                    const best = await db.get(`SELECT r.wind FROM result r JOIN heat h ON h.id=r.heat_id WHERE h.event_id=? AND r.event_entry_id=? AND r.distance_meters>0 ORDER BY r.distance_meters DESC, r.attempt_number DESC LIMIT 1`, se.id, subEntry.id);
                    if (best && typeof best.wind === 'number') winds.push(best.wind); else missing = true;
                } else {
                    const row = await db.get(`SELECT r.wind AS rw, h.wind AS hw FROM result r JOIN heat h ON h.id=r.heat_id WHERE h.event_id=? AND r.event_entry_id=? AND r.time_seconds>0 LIMIT 1`, se.id, subEntry.id);
                    const w = row ? effectiveWind({ wind: row.rw }, { wind: row.hw }) : null;
                    if (w != null) winds.push(w); else missing = true;
                }
            }
            if (!missing && winds.length > 0) {
                const avg = winds.reduce((a, b) => a + b, 0) / winds.length;
                if (avg > WIND_LIMIT + 1e-9) return { detected: [], skipped: `combined-wind-avg:${avg.toFixed(2)}m/s (>${WIND_LIMIT.toFixed(1)}, 참고기록)` };
            }
        } catch (e) { /* 풍속 판정 실패는 감지를 막지 않는다 */ }
        ...
        async function _logCombined(record_type, division_code, series_id) {
            ...
            const existingNum = existing ? parseRecordValue(existing.record_value) : null;
            if (!existing || existingNum == null) return null; // 기준 기록 없음 → 감지 안 함 (개별 종목과 동일)
            const tie = !isBetter(totalPoints, existingNum, direction) && isTie(totalPoints, existingNum, direction);
            if (!tie && !isBetter(totalPoints, existingNum, direction)) return null;
        ...
        // 2) DR — 부 라벨 → 부 마스터 코드
        const divCode = await resolveDivisionCode(db, parent_event);
        if (divCode) {
            const drRes = await _logCombined('division', divCode, null);
            if (drRes) detected.push(drRes);
        }
