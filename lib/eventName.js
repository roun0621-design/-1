...
const DIVISION_SUFFIX_RE = /\s+(?:남자|여자)?(?:초등학교부|초등부|중학교부|중학부|중등부|고등학교부|고등부|대학교부|대학부|일반부|실업부|공개부|통합부|(?:초|중|고|중등|고등|초등)\s*\d학년부?|U(?:18|20)(?:\([남여혼]\))?|선수권(?:\([남여혼]\))?|국제)$/i;
/** '100m 일반부' → '100m' (부 접미가 없으면 그대로). 기록 대조·연맹 기록지 종목 칸에 쓴다 */
function stripDivisionSuffix(name) { return String(name || '').replace(DIVISION_SUFFIX_RE, '').trim(); }
...
function recordKey(name) {
    ...
    // 부 접미 제거 (2026-09 Phase 7-②): 한 대회에 여러 부가 있으면 종목명에 부를 붙여 저장한다('100m 일반부'·'1500m 중1학년부', 종목 UNIQUE 때문).
    // 기록표는 '100m' 으로 있으므로 마지막 낱말이 부 라벨이면 뗀다 — 예전엔 이런 종목의 신기록이 한 번도 대조되지 않았다
    s = stripDivisionSuffix(s);
...
module.exports = {
    stripDivisionSuffix, storedName, recordKey, sameEvent };
