...
function buildGetEventResultsForCert(db) {
    ...
    return async function getEventResultsForCert(eventId) {
        ...
        // 순위 → 상태코드(NM → DNF → DQ → DNS, 공용 규칙) — 기록 없는 선수끼리도 같은 순서
        const order = [...ranked.sort((a, b) => (a.rank - b.rank)), ...rows.filter(r => r.best == null).sort(PaceRanking.withStatusLast(() => 0))];
...
module.exports = function mountCertificateRoutes(app, deps) {
    ...
    // ── 워드 상장 양식 (현장 인쇄용) — PDF 양식(certificate_template)과 별개 ─────────────────
    //   GET    /api/award-docx-template?competition_id=   적용되는 양식(대회 → 전체 기본 → 내장 기본값) + source
    //   PUT    /api/award-docx-template                   { competition_id|null, config } 저장 (관리자)
    //   DELETE /api/award-docx-template?competition_id=   대회 양식 삭제 → 전체 기본으로 되돌림 (관리자)
    //   POST   /api/award-docx-template/preview           { config, competition_id } 저장 전 미리보기 .docx (관리자)
    const AwardTpl = require('../awardDocxTemplate');
    const _awardTplReady = Promise.resolve().then(() => AwardTpl.ensureTable(db)).catch(e => console.error('[award-docx-template] table:', e.message));
    const _keyOfReq = req => (req.body && req.body.admin_key) || req.headers['x-admin-key'] || (req.query && (req.query.admin_key || req.query.key)) || '';
    app.get('/api/award-docx-template', async (req, res) => {
        try {
            const k = _keyOfReq(req);
            if (!(isAdminKey(k) || (typeof isOperationKey === 'function' && isOperationKey(k)))) return res.status(403).json({ error: '운영키 또는 관리자 권한이 필요합니다.' });
            await _awardTplReady;
            const compId = req.query.competition_id ? parseInt(req.query.competition_id, 10) : null;
            res.json({ ...(await AwardTpl.getEffective(db, compId)), competition_id: compId, defaults: AwardTpl.DEFAULTS, fonts: AwardTpl.FONTS });
        } catch (e) { res.status(500).json({ error: e.message }); }
    });
    app.put('/api/award-docx-template', async (req, res) => {
        try {
            if (!isAdminKey(_keyOfReq(req))) return res.status(403).json({ error: '관리자 권한이 필요합니다.' });
            await _awardTplReady;
            const compId = req.body.competition_id ? parseInt(req.body.competition_id, 10) : null;
            const config = await AwardTpl.save(db, compId, req.body.config || {});
            res.json({ success: true, source: compId ? 'competition' : 'global', competition_id: compId, config });
        } catch (e) { res.status(500).json({ error: e.message }); }
    });
    app.delete('/api/award-docx-template', async (req, res) => {
        try {
            if (!isAdminKey(_keyOfReq(req))) return res.status(403).json({ error: '관리자 권한이 필요합니다.' });
            await _awardTplReady;
            const compId = (req.query.competition_id || (req.body && req.body.competition_id)) ? parseInt(req.query.competition_id || req.body.competition_id, 10) : null;
            await AwardTpl.remove(db, compId);
            res.json({ success: true, ...(await AwardTpl.getEffective(db, compId)) });
        } catch (e) { res.status(500).json({ error: e.message }); }
    });
    app.post('/api/award-docx-template/preview', async (req, res) => {
        try {
            if (!isAdminKey(_keyOfReq(req))) return res.status(403).json({ error: '관리자 권한이 필요합니다.' });
            const comp = req.body.competition_id ? await db.get('SELECT name FROM competition WHERE id=?', req.body.competition_id) : null;
            const compName = (comp && comp.name) || '제00회 전국육상경기대회';
            const sample = [
                { athlete_name: '홍길동', team: '○○시청', gender: 'M', division: '일반', event_name: '100m', rank: 1, record_value: '10.31', wind: 1.2, competition_name: compName },
                { athlete_name: '○○시청', team: '○○시청', gender: 'F', division: '일반', event_name: '4x100mR', rank: 2, record_value: '46.52', wind: null, members: ['김하나', '이두리', '박세나', '최네오'], competition_name: compName },
            ];
            const buf = await require('../certificateDocx').generateCertificateDocx(AwardTpl.normalize(req.body.config || {}), sample);
            const fileName = encodeURIComponent('상장_미리보기.docx');
            res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document');
            res.setHeader('Content-Disposition', `attachment; filename="${fileName}"; filename*=UTF-8''${fileName}`);
            res.end(buf);
        } catch (e) { res.status(500).json({ error: e.message }); }
    });
    // 계주: 수상자는 팀 — 상장에 선수 명단을 함께 적는다
    async function _relayMembersOf(entryId) {
        try {
            const rows = await db.all(`SELECT a.name FROM relay_member rm JOIN athlete a ON a.id = rm.athlete_id WHERE rm.event_entry_id=? ORDER BY CASE WHEN rm.leg_order IS NULL THEN 99 ELSE rm.leg_order END, rm.id`, entryId);
...
<body><div class="box"><h1>상장을 열 수 없습니다</h1><p>${msg}</p></div></body></html>`;
    ...
    app.post('/api/admin/certificates/generate', async (req, res) => {
        try {
            ...
            // 워드는 '워드 상장 양식'을 쓴다 (PDF 양식과 별개)
            let buf;
            if (wantDocx) { await _awardTplReady; buf = await require('../certificateDocx').generateCertificateDocx((await AwardTpl.getEffective(db, competition_id)).config, items); }
            else buf = await generateCertificateBatch(tpl, items);
    ...
    app.post('/api/certificates/event-award', async (req, res) => {
        try {
            ...
            // 워드(기본): '워드 상장 양식' — 저장된 게 없어도 내장 기본 양식으로 바로 나온다. PDF 를 고른 경우에만 PDF 양식이 필요하다.
            let tpl = null;
            if (wantPdf) {
                tpl = template_id
                ...
                if (!tpl) return res.status(404).json({ error: 'PDF 시상 양식이 없습니다. 관리자 → 상장관리에서 양식을 먼저 만드세요.' });
            }
            ...
            const items = rows.filter(r => r.rank != null && r.rank >= from && r.rank <= to).map(r => ({
                athlete_id: r.athlete_id, entry_id: r.entry_id, athlete_name: r.athlete_name, team: r.team, bib_number: r.bib_number,
            ...
            let buf;
            if (wantPdf) buf = await generateCertificateBatch(tpl, items);
            else {
                await _awardTplReady;
                if (event.category === 'relay') for (const it of items) it.members = await _relayMembersOf(it.entry_id);
                buf = await require('../certificateDocx').generateCertificateDocx((await AwardTpl.getEffective(db, event.competition_id)).config, items);
            }
            try {
                const now = new Date().toISOString();
                for (const it of items) await db.run(`INSERT INTO certificate_issue_log (competition_id, template_id, event_id, athlete_id, rank_value, record_value, issued_at, issued_by, note) VALUES (?,?,?,?,?,?,?,?,?)`,
                    event.competition_id, tpl ? tpl.id : 0, event.id, it.athlete_id, it.rank, it.record_value || '', now, '기록입력', wantPdf ? 'award' : 'award-docx');
