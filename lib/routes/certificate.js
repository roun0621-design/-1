    ...
    return async function getEventResultsForCert(eventId) {
        ...
        const ranked = list.map((row, idx) => {
            ...
            if (row.time_seconds && row.time_seconds > 0) {
                ...
                if (t >= 60) {
                    const r = t - Math.floor(t / 60) * 60;
                    const m = Math.floor(t / 60);
                    const dp = _secDp(r);
                    const s = r.toFixed(dp);
                    recordValue = `${m}:${s.padStart(dp === 3 ? 6 : 5, '0')}`;
                } else {
                    recordValue = t.toFixed(_secDp(t));
                }
            ...
            return {
                ...
                wind: (row.wind != null && row.wind !== '') ? row.wind : null,
                finished: hasResult && (!row.status_code || row.status_code === 'OK'),
                status_code: row.status_code,
                heat_number: null,   // 아래에서 채움
                heat_rank: null,     // 조 내 순위 (아래에서 계산)
            };
        });

        // ── 조(heat) 번호 매핑 + 조별 순위 계산 ──
        // 기존 종합순위 로직은 그대로 두고, entry_id → heat_number 를 별도 조회해 부여
        // (조편성 재배정/재업로드에도 안전하도록 heat_entry 기준).
        const heatMap = new Map();
        const heRows = await db.all(`
            SELECT he.event_entry_id AS entry_id, hh.heat_number AS heat_number
            FROM heat_entry he
            JOIN heat hh ON hh.id = he.heat_id
            WHERE hh.event_id = ?
        `, eventId);
        for (const hr of heRows) if (!heatMap.has(hr.entry_id)) heatMap.set(hr.entry_id, hr.heat_number);

        // 종합순위(ranked)는 이미 기록순 정렬 → 조별로 카운트하면 조 내 상대순위가 그대로 보존됨
        const heatCounters = new Map();
        for (const r of ranked) {
            r.heat_number = heatMap.has(r.entry_id) ? heatMap.get(r.entry_id) : null;
            if (r.rank == null) { r.heat_rank = null; continue; }  // 실격/기권 등은 순위 없음
            const hk = (r.heat_number == null) ? '_' : r.heat_number;
            const c = (heatCounters.get(hk) || 0) + 1;
            heatCounters.set(hk, c);
            r.heat_rank = c;
        }
        const heatCount = new Set(ranked.map(r => r.heat_number).filter(x => x != null)).size;

        return { event, rows: ranked, heatCount };
    };
}
    ...
    app.post('/api/admin/certificates/generate', async (req, res) => {
        try {
            const { admin_key, template_id, competition_id, event_ids,
                    rank_from, rank_to, include_finishers, mode, rank_scope } = req.body || {};
