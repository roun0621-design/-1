...
module.exports = function mountPushRoutes(app, deps) {
    ...
    app.post('/api/push/register', async (req, res) => {
        try {
            const { token, audience, competition_id, platform } = req.body || {};
            if (!token) return res.status(400).json({ error: 'token 필요' });
            const aud = audience === 'staff' ? 'staff' : 'public';
            const plat = ['ios', 'android', 'web'].includes(String(platform || '').toLowerCase()) ? String(platform).toLowerCase() : 'web';   // 네이티브 앱 브리지가 보낸 토큰
            const ua = String(req.headers['user-agent'] || '').slice(0, 300);
            const now = new Date().toISOString();
            const existing = await db.get('SELECT id FROM push_token WHERE token=?', token);
            if (existing) {
                await db.run('UPDATE push_token SET audience=?, competition_id=?, user_agent=?, platform=?, active=1, updated_at=? WHERE token=?',
                    aud, competition_id || null, ua, plat, now, token);
            } else {
                await db.run('INSERT INTO push_token (token, audience, competition_id, user_agent, platform, active, created_at, updated_at) VALUES (?, ?, ?, ?, ?, 1, ?, ?)',
                    token, aud, competition_id || null, ua, plat, now, now);
            }
    ...
    app.post('/api/admin/push/send', async (req, res) => {
        try {
            ...
            // 대상 토큰 조회
            let sql = 'SELECT token, platform FROM push_token WHERE active=1';
            const params = [];
            ...
            const tokens = rows.map(r => ({ token: r.token, platform: r.platform || 'web' }));
            if (!tokens.length) {
            ...
            const r = await Push.sendToTokens(tokens, {
                title: title || '알림',
                body,
                data: { competition_id: String(competition_id || ''), audience: aud, url: competition_id ? `/dashboard.html?comp=${competition_id}` : '/' },
            });
    ...
    async function notifyEventInterest(event, opts) {
        try {
            ...
            const rows = await db.all(
                `SELECT DISTINCT pt.token AS token, pt.platform AS platform
            ...
            const r = await Push.sendToTokens(tokens, {
                ...
                body: (opts && opts.body) || '',
                data: { event_id: String(event.id || ''), kind: (opts && opts.kind) || '', url: `/dashboard.html?comp=${event.competition_id}` },
            });
            if (r.invalidTokens && r.invalidTokens.length) {
                const now = new Date().toISOString();
