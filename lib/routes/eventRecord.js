...
module.exports = function mountEventRecordRoutes(app, deps) {
    ...
    const normPhone = (p) => SMS.normalizePhone(p);      // '+8210…' 도 '010…' 으로 (예전엔 숫자만 남겨 82 가 앞에 붙은 채 다른 사람으로 저장됐다)
        ...
        try {
            ...
            const row = await db.get(
                `SELECT ee.id AS entry_id, ee.event_id, a.id AS athlete_id, a.name, a.team, a.phone,
                 ...
                 WHERE ee.id = ? AND e.competition_id = ?`, entry_id, comp.id);      // 이 행사의 참가자만 — 예전엔 다른 대회 선수에게도 이 행사 이름으로 문자가 나갈 수 있었다
            ...
            if (!SMS.isValidMobile(phone)) return res.status(400).json({ error: `휴대폰 번호 형식이 아닙니다: ${row.phone}` });

            const cfg = await db.get('SELECT * FROM sms_config WHERE id=1');
            if (!cfg) return res.status(500).json({ error: 'SMS 설정이 초기화되지 않았습니다.' });
            if (cfg.monthly_quota && !cfg.sim_mode && cfg.api_key && (cfg.sent_this_month || 0) + 1 > cfg.monthly_quota) return res.status(429).json({ error: `이번 달 문자 한도(${cfg.monthly_quota}건)를 넘습니다.`, quota_exceeded: true });

            // 같은 참가자에게 2분 안에 다시 보내지 않는다 — 버튼 연타·응답 유실 뒤 재시도로 유료 문자가 두 번 나가는 것을 막는다 (force:true 로 재발송)
            if (!(req.body && req.body.force)) {
                const recent = await db.get(`SELECT sent_at FROM sms_log WHERE competition_id=? AND athlete_id=? AND triggered_by='event-record' AND status IN ('sent','simulated') ORDER BY id DESC LIMIT 1`, comp.id, row.athlete_id);
                if (recent && recent.sent_at && Date.now() - new Date(recent.sent_at).getTime() < 2 * 60 * 1000) {
                    return res.status(409).json({ error: '방금 이 참가자에게 발송했습니다. 잠시 뒤 다시 시도하세요.', already_sent: true });
                }
            }
