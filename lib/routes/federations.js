...
module.exports = function mountFederationRoutes(app, deps) {
    ...
    app.put('/api/federations/:id', async (req, res) => {
        const { admin_key, code, name, badge_bg, badge_color, sort_order, gender_label_m, gender_label_f, gender_label_x, hidden } = req.body;
        if (!isAdminKey(admin_key)) return res.status(403).json({ error: '관리자 키가 필요합니다.' });
        const old = await db.get('SELECT * FROM federation_list WHERE id=?', req.params.id);
        if (!old) return res.status(404).json({ error: 'Not found' });
        try {
            const hiddenVal = hidden === undefined ? (old.hidden ? 1 : 0) : ((hidden === true || hidden === 1 || hidden === '1' || hidden === 'true') ? 1 : 0);
            await db.run(
                'UPDATE federation_list SET code=?, name=?, badge_bg=?, badge_color=?, sort_order=?, gender_label_m=?, gender_label_f=?, gender_label_x=?, hidden=? WHERE id=?',
    ...
    // 연맹 숨김 토글 — 숨기면 그 연맹 소속 대회 전체가 홈·운영 화면 목록에서 빠진다 (관리자 페이지에서만 보임)
    app.put('/api/federations/:id/hidden', async (req, res) => {
        const { admin_key, hidden } = req.body;
        if (!isAdminKey(admin_key)) return res.status(403).json({ error: '관리자 키가 필요합니다.' });
        const old = await db.get('SELECT * FROM federation_list WHERE id=?', req.params.id);
        if (!old) return res.status(404).json({ error: 'Not found' });
        const val = (hidden === true || hidden === 1 || hidden === '1' || hidden === 'true') ? 1 : 0;
        await db.run('UPDATE federation_list SET hidden=? WHERE id=?', val, old.id);
        opLog(`${val ? '연맹 숨김' : '연맹 숨김 해제'}: ${old.code}`, 'admin', 'admin');
        res.json(await db.get('SELECT * FROM federation_list WHERE id=?', old.id));
    });
