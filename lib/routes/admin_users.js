...
module.exports = function mountAdminUsersRoutes(app, deps) {
    ...
    // -----------------------------------------------------------
    // GET /api/admin/login-audit?limit=200&only=fail — 로그인 이력 (JWT 계정 + 레거시 키 로그인)
    app.get('/api/admin/login-audit', async (req, res) => {
        const me = await requireAdmin(req, res);
        if (!me) return;
        const limit = Math.min(Math.max(parseInt(req.query.limit, 10) || 200, 1), 1000);
        const onlyFail = req.query.only === 'fail';
        try {
            const rows = await db.all(
                `SELECT id, user_id, username, success, failure_reason, ip, user_agent, created_at
                 FROM login_audit ${onlyFail ? 'WHERE success = 0' : ''} ORDER BY id DESC LIMIT ${limit}`
            );
            const locked = await db.all(`SELECT id, username, failed_attempts, locked_until FROM app_user WHERE locked_until IS NOT NULL AND locked_until > ?`, new Date().toISOString());
            res.json({ rows, locked });
    ...
    app.post('/api/admin/users/:id/unlock', async (req, res) => {
        const me = await requireAdmin(req, res);
        if (!me) return;
        await db.run('UPDATE app_user SET failed_attempts=0, locked_until=NULL WHERE id=?', req.params.id);
        res.json({ ok: true });
    });
