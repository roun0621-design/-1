...
app.get('/api/timetable/:compId', async (req, res) => {
    ...
    for (const r of rows) {
        ...
        days[r.day][s].push({ spotlight: spotEvents && r.event_id && spotEvents.has(Number(r.event_id)) ? spotlight : null, id: r.id, time: r.time, event_name: r.event_name, category: r.category, round: r.round, note: r.note, event_id: r.event_id, callroom_time: r.callroom_time, scheduled_date: r.scheduled_date, result_url, round_status });
    }
