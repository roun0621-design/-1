...
module.exports = function mountTimetableRoutes(app, deps) {
    const { db, isAdminKey, isOperationKey, opLog, upload, XLSX, excelTimeToHHMM, cleanTimetableEventName } = deps;
    if (typeof excelTimeToHHMM !== 'function' || typeof cleanTimetableEventName !== 'function') {
        throw new Error('[timetable.js] mount requires { excelTimeToHHMM, cleanTimetableEventName } — Excel 업로드 파싱 헬퍼 누락');
    }
...
app.get('/api/timetable/:compId', async (req, res) => {
    ...
    const comp = await db.get('SELECT start_date, sync_source FROM competition WHERE id=?', req.params.compId);
    // 국제대회: 관심 국가(KOR) 선수가 출전하는 종목의 행에 표시 (같은 종목의 다른 라운드도 함께)
    let spotEvents = null, spotlight = null;
    try {
        const src = comp && comp.sync_source ? JSON.parse(comp.sync_source) : null;
        if (src && src.spotlight) {
            spotlight = src.spotlight;
            const ev = await db.all(`SELECT DISTINCT e.id, e.external_key FROM event e JOIN event_entry ee ON ee.event_id=e.id JOIN athlete a ON a.id=ee.athlete_id WHERE e.competition_id=? AND a.team=?`, req.params.compId, spotlight);
            const bases = new Set(ev.map(x => String(x.external_key || '').split('#')[0]).filter(Boolean));
            const all = await db.all('SELECT id, external_key FROM event WHERE competition_id=?', req.params.compId);
            spotEvents = new Set([...ev.map(x => Number(x.id)), ...all.filter(x => x.external_key && bases.has(String(x.external_key).split('#')[0])).map(x => Number(x.id))]);
        }
    } catch (e) {}
    // 행별 출전 인원·레인 배정 인원 — 경기 전 행에 '엔트리'/'명단' 표시용 (한 번에 집계)
    const entryCount = new Map(), laneCount = new Map();
    try {
        for (const x of await db.all('SELECT ee.event_id, COUNT(*) AS c FROM event_entry ee JOIN event e ON e.id=ee.event_id WHERE e.competition_id=? GROUP BY ee.event_id', req.params.compId)) entryCount.set(Number(x.event_id), Number(x.c));
        for (const x of await db.all('SELECT h.event_id, COUNT(he.id) AS c FROM heat h JOIN heat_entry he ON he.heat_id=h.id JOIN event e ON e.id=h.event_id WHERE e.competition_id=? GROUP BY h.event_id', req.params.compId)) laneCount.set(Number(x.event_id), Number(x.c));
    } catch (e) {}
    // Group by day
    const days = {};
    for (const r of rows) {
        ...
        if (r.section === 'ceremony') continue;     // 시상식은 시간표에 싣지 않는다 (국제대회 동기화가 넣던 것)
        days[r.day][s].push({ spotlight: spotEvents && r.event_id && spotEvents.has(Number(r.event_id)) ? spotlight : null, entry_count: r.event_id ? (entryCount.get(Number(r.event_id)) || 0) : 0, lane_count: r.event_id ? (laneCount.get(Number(r.event_id)) || 0) : 0, id: r.id, time: r.time, event_name: r.event_name, category: r.category, round: r.round, note: r.note, event_id: r.event_id, callroom_time: r.callroom_time, scheduled_date: r.scheduled_date, result_url, round_status });
    }
...
app.post('/api/timetable/upload', upload.single('file'), async (req, res) => {
    try {
        ...
        // ─── 완전 동일 행(day·section·시간·종목·종별·라운드) 중복 제거 ───
        //   DB UNIQUE(competition_id,day,section,time,event_name,category,round) 위반으로
        //   업로드 전체가 실패하던 문제 방지. 중복은 첫 행만 남기고 제거(개수 안내).
        //   ※ 종별(부)이 다르면(예: 여자일반부 vs 여자고등부) 같은 시간이어도 중복 아님 — 정상 허용.
        let dupDropped = 0;
        {
            const _seen = new Set();
            allEntries = allEntries.filter(e => {
                const k = `${e.day}|${e.section}|${e.time}|${e.event_name}|${e.category}|${e.round}`;
                if (_seen.has(k)) { dupDropped++; return false; }
                _seen.add(k);
                return true;
            });
        }
        ...
        // (2026-09) 파일의 모든 일차가 '지난 날'이라 아무것도 반영되지 않은 경우 — 예전엔 성공 메시지("보존")만 떠서
        //   올린 사람이 반영된 줄 알았다. 대회가 끝난 뒤 정정본을 올릴 때마다 생기는 상황 → 분명히 알리고 강제 모드를 안내한다.
        const nothingApplied = overwriteMode !== 'force' && skippedPastDays.length > 0
            && !mergeStats.addedCount && !mergeStats.updatedCount && !mergeStats.deletedCount && effectiveDays.length === 0;
        if (nothingApplied) msg = `반영된 내용이 없습니다 — 파일의 ${skippedPastDays.map(d=>d+'일차').join('·')}는 이미 지난 날짜라 보호되었습니다. 지난 일차를 고치려면 '강제 덮어쓰기'로 다시 올리세요.`;
        if (dupDropped) msg += ` · 중복 ${dupDropped}행 자동제외`;
        ...
        res.json({
            ...
            skippedPastDays,
            mode: overwriteMode,
            nothing_applied: nothingApplied,
            needs_force: nothingApplied,
...
async function autoLinkTimetable(compId, options = {}) {
    ...
    // 이름 정규화·종별 해석은 공통 규칙 (lib/eventMatch.js)
    const { parseCategory, combinedParentName, findEvents, pickByGender } = EM;
    ...
    function parseRound(roundStr, eventName) {
        ...
        // Combined event sub-events: "10종(N)", "7종(N)", "5종(N)" → 부모 종목
        const parent = combinedParentName(r);
        if (parent) return { round: 'final', isCombinedSub: true, parentName: parent };
    ...
    for (const tt of ttRows) {
        ...
        const targetName = parsed.isCombinedSub ? parsed.parentName : tt.event_name;

        // A7: 이름·라운드·성별이 맞는 종목 전부 (남녀 동시출발 행은 두 종목) — 공통 규칙
        const { matches } = findEvents(compEvents, { name: targetName, genders: catInfo.genders, round: ttRound });

        if (matches.length > 0) {
            // 중복 종목 robustness: 성별마다 데이터 가장 많은 종목을 대표로 (기록 > 엔트리 > 조)
            const representatives = pickByGender(matches, e => evScoreMap.get(e.id) || 0);
...
// server.js 의 autoLinkDisplayTimetable 이 결승/준결승 생성 직후 폴백으로 재사용 (업로드·재매칭과 동일 로직)
return { autoLinkTimetable };
};
