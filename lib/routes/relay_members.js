...
module.exports = function mountRelayMemberRoutes(app, deps) {
    ...
    app.post('/api/relay-members', async (req, res) => {
        ...
        try {
            // 혼성(X) 계주는 남 2·여 2 (WA TR 24 · 중고연맹 요강) — 세 번째 같은 성별은 거부
            const ev = await db.get('SELECT e.gender, e.category, e.name FROM event_entry ee JOIN event e ON e.id=ee.event_id WHERE ee.id=?', event_entry_id);
            if (ev && ev.gender === 'X' && require('../kjafRules').isRelay(ev)) {
                const cur = await db.all('SELECT a.gender FROM relay_member rm JOIN athlete a ON a.id=rm.athlete_id WHERE rm.event_entry_id=? AND rm.athlete_id<>?', event_entry_id, athlete_id);
                const add = await db.get('SELECT gender FROM athlete WHERE id=?', athlete_id);
                const chk = require('../kjafRules').checkMixedRelay(cur.map(x => x.gender), add && add.gender);
