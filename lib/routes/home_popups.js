...
module.exports = function mountHomePopupRoutes(app, deps) {
    ...
    app.get('/api/home-popups', async (req, res) => {
        // ?competition_id=X → 그 대회 전용 팝업만 (대시보드 공지 버튼/자동노출용)
        // ?competition_id=common → 공통(전체) 팝업만 (홈 자동노출용)
        // 파라미터 없음 → 전체 (관리자 목록용)
        const cid = req.query.competition_id;
        let popups;
        if (cid === 'common' || cid === 'null') {
            popups = await db.all('SELECT * FROM home_popup WHERE competition_id IS NULL ORDER BY sort_order, id');
        } else if (cid !== undefined && cid !== '') {
            popups = await db.all('SELECT * FROM home_popup WHERE competition_id=? ORDER BY sort_order, id', cid);
        } else {
            popups = await db.all('SELECT * FROM home_popup ORDER BY sort_order, id');
        }
    ...
    app.post('/api/home-popups', async (req, res) => {
        const {
            ...
            is_active, show_from, show_until, sort_order, sections, competition_id
        } = req.body;
        ...
        try {
            ...
            // 빈 값/0 → NULL(공통), 그 외 정수면 특정 대회
            const compId = (competition_id === undefined || competition_id === null || competition_id === '' || competition_id === 0 || competition_id === '0') ? null : parseInt(competition_id, 10);
            const info = await db.run(
                `INSERT INTO home_popup (popup_type, title, subtitle, intro_text, bottom_btn_text, bottom_btn_desc, bottom_btn_link, bottom_btn_active, is_active, show_from, show_until, sort_order, competition_id) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)`,
                ...
                show_from || null, show_until || null, sort_order ?? maxOrder + 1, compId
            );
    ...
    app.put('/api/home-popups/:id', async (req, res) => {
        ...
        try {
            ...
            // competition_id 미전달(undefined) → 기존값 유지, 전달됐으면 빈값/0 은 NULL(공통)
            const compId = (competition_id === undefined) ? old.competition_id
                : ((competition_id === null || competition_id === '' || competition_id === 0 || competition_id === '0') ? null : parseInt(competition_id, 10));
            await db.run(
                `UPDATE home_popup SET popup_type=?, title=?, subtitle=?, intro_text=?, bottom_btn_text=?, bottom_btn_desc=?, bottom_btn_link=?, bottom_btn_active=?, is_active=?, show_from=?, show_until=?, sort_order=?, competition_id=?, updated_at=${_nowF1} WHERE id=?`,
                ...
                sort_order ?? old.sort_order ?? 0, compId, old.id
            );
            if (Array.isArray(sections)) {
