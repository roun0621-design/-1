...
module.exports = function mountJointGroupsRoutes(app, deps) {
    ...
    const _norm = s => (s || '').replace(/\s+/g, '').toLowerCase().replace(/[×xX]/g, 'x');

    /**
     * 종합경기(7종/10종 등) 부모를 합동으로 묶으면, 같은 이름의 세부종목(100mH·멀리뛰기 등)도
     * 대회 간에 짝지어 각각 별도의 합동 그룹 + joint_scoreboard_key 를 자동 생성한다.
     * (부모=대단락만 묶이고 세부종목은 안 묶이던 문제 해결)
     *
     * @param {Array} parentEvents  합동 그룹에 묶인 부모 종목들 (id, competition_id, gender, round_type, category, name 포함)
     * @returns {number} 새로 생성된 세부종목 합동 그룹 수
     * ⚠️ 반드시 db.transaction(...) 내부에서 호출할 것 (자체 트랜잭션을 열지 않음)
     */
    async function expandCombinedSubEvents(parentEvents) {
        const combinedParents = (parentEvents || []).filter(e => e && e.category === 'combined');
        if (combinedParents.length < 2) return 0; // 2개 대회 이상의 종합경기라야 합동 의미 있음

        // 모든 세부종목 수집
        const subs = [];
        for (const p of combinedParents) {
            const rows = await db.all('SELECT id, name, gender, round_type, competition_id FROM event WHERE parent_event_id=?', p.id);
            rows.forEach(r => subs.push(r));
        }

        // 세부종목명(정규화) 기준으로 그룹화
        const byName = {};
        for (const s of subs) {
            const k = _norm(s.name);
        ...
        for (const list of Object.values(byName)) {
            // 서로 다른 대회 2곳 이상에 존재해야 함 — 대회당 1개로 dedupe
            const perComp = {};
            for (const s of list) { if (!perComp[s.competition_id]) perComp[s.competition_id] = s; }
            const members = Object.values(perComp);
            if (members.length < 2) continue;

            // 이미 동일 세부종목들이 묶인 그룹이 있으면 skip (재실행 안전)
            const ids = members.map(m => m.id);
            const existing = await db.get(`SELECT jg.id FROM joint_group jg
                JOIN joint_group_member jgm ON jgm.joint_group_id=jg.id
                WHERE jgm.event_id IN (${ids.map(() => '?').join(',')})
                GROUP BY jg.id HAVING COUNT(*)>=2`, ...ids);
            if (existing) continue;

            const e0 = members[0];
            const genderLabel = { M: '남자', F: '여자', X: '혼성' }[e0.gender] || '';
            const roundLabel = { preliminary: '예선', semifinal: '준결승', final: '결승' }[e0.round_type] || '';
            const subKey = `합동 ${genderLabel} ${e0.name} ${roundLabel}`.trim();

            const gInfo = await db.run('INSERT INTO joint_group (name, joint_scoreboard_key) VALUES (?, ?)', e0.name, subKey);
            for (let i = 0; i < members.length; i++) {
                await db.run('INSERT OR IGNORE INTO joint_group_member (joint_group_id, event_id, competition_id, sort_order) VALUES (?, ?, ?, ?)', gInfo.lastInsertRowid, members[i].id, members[i].competition_id, i);
            }
            // backward-compat event_link (pair-wise)
            for (let i = 0; i < members.length; i++) {
                for (let j = i + 1; j < members.length; j++) {
                    const [idA, idB] = members[i].id < members[j].id ? [members[i].id, members[j].id] : [members[j].id, members[i].id];
                    await db.run('INSERT OR IGNORE INTO event_link (event_id_a, event_id_b, joint_scoreboard_key) VALUES (?, ?, ?)', idA, idB, subKey);
                }
            }
            createdSub++;
        }
...
app.post('/api/joint-groups', async (req, res) => {
    ...
    await db.transaction(async () => {
        ...
        // 종합경기(7종/10종)면 세부종목도 자동으로 합동 묶기
        subCreated = await expandCombinedSubEvents(events);
    })();

    const feds = events.map(e => e.federation || e.comp_name).join('+');
    opLog(`합동 그룹 생성: ${groupName} (${feds}, ${events.length}개 종목${subCreated ? `, 세부종목 ${subCreated}개 합동` : ''})`, 'admin', 'admin');
...
app.post('/api/joint-groups/:id/members', async (req, res) => {
    ...
    await db.transaction(async () => {
        ...
        // 종합경기면 세부종목도 자동 합동 (그룹의 모든 부모 멤버 기준)
        const allMembers = await db.all(`SELECT e.id, e.competition_id, e.gender, e.round_type, e.category, e.name
            FROM joint_group_member jgm JOIN event e ON e.id=jgm.event_id WHERE jgm.joint_group_id=?`, g.id);
        subCreated = await expandCombinedSubEvents(allMembers);
    })();
    res.json({ success: true, added, sub_groups_created: subCreated });
});
...
app.post('/api/joint-groups/auto-create', async (req, res) => {
    ...
    await db.transaction(async () => {
        for (const [key, events] of Object.entries(eventMap)) {
            ...
            if (existing) {
                // 부모는 이미 합동돼 있어도 세부종목이 안 묶였을 수 있으니 백필 (idempotent)
                subCreated += await expandCombinedSubEvents(events);
                continue;
            }
    ...
    opLog(`합동 그룹 자동 생성: ${created}개 그룹${subCreated ? ` + 세부종목 ${subCreated}개` : ''} (대회 ${competition_ids.join(', ')})`, 'admin', 'admin');
