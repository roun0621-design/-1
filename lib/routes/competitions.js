...
module.exports = function mountCompetitionsRoutes(app, deps) {
    ...
    app.post('/api/admin/competitions/:id/event-settings', async (req, res) => {
        try {
            const { admin_key, event_slug, brand_color_point, brand_color_accent, set_event_mode } = req.body || {};
            if (!isAdminKey(admin_key)) return res.status(403).json({ error: '관리자 키가 필요합니다.' });
            ...
            await db.run('UPDATE competition SET mode=?, event_slug=?, brand_color_point=?, brand_color_accent=? WHERE id=?',
                mode, slug, (brand_color_point || ''), (brand_color_accent || ''), comp.id);
