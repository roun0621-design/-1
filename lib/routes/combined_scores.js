...
module.exports = function mountCombinedScoresRoutes(app, deps) {
    const { db, isAdminKey, isOperationKey, opLog, broadcastSSE,
            ...
            requireAdminAfterCompEnd } = deps;
    if (!app || !db || !isAdminKey || !isOperationKey || !opLog || !broadcastSSE
        || typeof requireAdminAfterCompEnd !== 'function') {
        throw new Error('[combined_scores.js] mount requires { db, isAdminKey, isOperationKey, opLog, broadcastSSE, requireAdminAfterCompEnd }');
    }
    ...
    app.post('/api/combined-scores/save', async (req, res) => {
        ...
        try {
            ...
            try {
                ...
                // 알려진 기능 갭: _runCombinedRecordCompareHook 은 results.js 에만 로컬 정의돼 있어
                // 이 모듈(combined_scores.js)에서는 보이지 않는다. 직접 입력 경로의 혼성 신기록 자동감지는
                // 현재 미작동(비치명적). typeof 가드로 삼켜지는 ReferenceError 를 제거. (추후 공용화 필요)
                if (subEntry && subEntry.parent_event_id && typeof _runCombinedRecordCompareHook === 'function') {
    ...
    app.post('/api/combined-scores/sync', async (req, res) => {
        ...
        // 🛡️ 자식(세부)종목 category 자동 교정 — 세부종목은 track/field_distance/field_height 만 유효.
        // 'combined' 등 잘못 저장된 트랙 세부종목(100mH/200m/800m)은 기록입력 화면 라우팅이 깨져
        // (종합순위 화면만 떠서 입력 불가) → 종목명으로 실제 분류를 추정해 교정한다.
        const _VALID_SUB_CATS = new Set(['track', 'field_distance', 'field_height']);
        const _guessSubCat = (nm) => {
            const n = (nm || '').replace(/^\[.*?\]\s*/, '').replace(/\s+/g, '');
            if (/높이뛰기|장대높이뛰기/.test(n)) return 'field_height';
            if (/멀리뛰기|세단뛰기|포환던지기|원반던지기|창던지기|해머던지기|던지기|뛰기/.test(n)) return 'field_distance';
            return 'track';
        };
        for (const se of subEvents) {
            if (!_VALID_SUB_CATS.has(se.category)) {
                const fixed = _guessSubCat(se.name);
                await db.run('UPDATE event SET category=? WHERE id=?', fixed, se.id);
                se.category = fixed;
                healedCount++;
            }
        }
