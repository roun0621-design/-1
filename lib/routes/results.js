...
module.exports = function mountResultsRoutes(app, deps) {
    const { db, isAdminKey, isOperationKey, opLog, broadcastSSE,
            calcWAPoints, requireAdminAfterCompEnd, audit } = deps;
    if (!app || !db || !isAdminKey || !isOperationKey || !opLog || !broadcastSSE
        || !calcWAPoints || !requireAdminAfterCompEnd || !audit) {
        throw new Error('[results.js] mount requires { db, isAdminKey, isOperationKey, opLog, broadcastSSE, calcWAPoints, requireAdminAfterCompEnd, audit }');
    }
