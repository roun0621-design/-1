...
module.exports = function mountResultsRoutes(app, deps) {
    const { db, isAdminKey, isOperationKey, opLog, broadcastSSE,
            calcWAPoints, requireAdminAfterCompEnd, audit, parseDbTimestampMs,
            DECATHLON_KEYS, HEPTATHLON_KEYS } = deps;
    if (!app || !db || !isAdminKey || !isOperationKey || !opLog || !broadcastSSE
        || !calcWAPoints || !requireAdminAfterCompEnd || !audit || typeof parseDbTimestampMs !== 'function'
        || !Array.isArray(DECATHLON_KEYS) || !Array.isArray(HEPTATHLON_KEYS)) {
        // DECATHLON_KEYS/HEPTATHLON_KEYS 누락 시 세부종목 기록 저장 후 _syncCombinedScoresForAthlete 가
        // ReferenceError 로 조용히 실패해 종합 점수가 자동 동기화되지 않았음 (2026-09 예천 검증에서 발견)
        throw new Error('[results.js] mount requires { db, isAdminKey, isOperationKey, opLog, broadcastSSE, calcWAPoints, requireAdminAfterCompEnd, audit, parseDbTimestampMs, DECATHLON_KEYS, HEPTATHLON_KEYS }');
    }
...
app.post('/api/results/upsert', async (req, res) => {
    ...
    try {
        ...
        if (existing) {
            ...
            let info;
            try {
                info = await db.run('INSERT INTO result (heat_id,event_entry_id,attempt_number,distance_meters,time_seconds,remark,status_code,wind) VALUES (?,?,?,?,?,?,?,?)', heat_id, event_entry_id, attempt_number || null, distance_meters ?? null, time_seconds ?? null, remark || '', sc || '', wind ?? null);
            } catch (insErr) {
                // 동시 요청이 먼저 같은 행을 만들었다(유니크 위반) → 그 행을 갱신하는 것으로 처리
                // SQLite: 'UNIQUE constraint failed' / PG: code 23505 (메시지는 서버 언어를 따라 한국어일 수 있다 → 코드로 판정)
                const _isDup = (insErr && (insErr.code === '23505' || insErr.code === 'SQLITE_CONSTRAINT_UNIQUE' || insErr.code === 'SQLITE_CONSTRAINT')) || /UNIQUE|duplicate key|고유 제약/i.test(String(insErr && insErr.message));
                if (!_isDup) throw insErr;
                const row = _attNum === null
                    ? await db.get('SELECT * FROM result WHERE heat_id=? AND event_entry_id=? AND attempt_number IS NULL', heat_id, event_entry_id)
                    : await db.get('SELECT * FROM result WHERE heat_id=? AND event_entry_id=? AND attempt_number=?', heat_id, event_entry_id, _attNum);
                if (!row) throw insErr;
                const _now = db.isAsync ? 'NOW()' : "datetime('now')";
                await db.run(`UPDATE result SET distance_meters=?,time_seconds=?,remark=?,status_code=?,wind=?,updated_at=${_now} WHERE id=?`,
                    distance_meters !== undefined ? (distance_meters ?? null) : row.distance_meters, time_seconds !== undefined ? (time_seconds ?? null) : row.time_seconds,
                    remark !== undefined ? (remark ?? '') : (row.remark ?? ''), status_code !== undefined ? sc : (row.status_code || ''), wind !== undefined ? (wind ?? null) : row.wind, row.id);
                info = { lastInsertRowid: row.id };
            }
...
async function _runRecordCompareHook(result, heatRow) {
    try {
        ...
        if (ret && ret.skipped && ret.skipped.startsWith('wind-over-limit')) {
            const wind = (typeof ret.wind === 'number') ? ret.wind : ((typeof result.wind === 'number') ? result.wind : null);
...
app.post('/api/results/reset-sub-event', async (req, res) => {
    const { event_id, include_joint } = req.body;
    if (!event_id) return res.status(400).json({ error: 'event_id required' });
    const evt = await db.get('SELECT * FROM event WHERE id=?', event_id);
    if (!evt) return res.status(404).json({ error: 'Event not found' });

    // 합동 종목(joint_group): 화면은 멤버 대회들의 기록을 한데 보여주는데 초기화는 이 종목의 조만 지워서
    // "초기화가 안 된다"고 보였음 → include_joint 이면 같은 그룹의 모든 멤버 종목 조를 함께 초기화.
    let targetEventIds = [Number(event_id)];
    let jointCount = 0;
    if (include_joint) {
        try {
            const jgm = await db.get('SELECT joint_group_id FROM joint_group_member WHERE event_id=?', event_id);
            if (jgm) {
                const members = await db.all('SELECT event_id FROM joint_group_member WHERE joint_group_id=?', jgm.joint_group_id);
                const ids = members.map(m => Number(m.event_id)).filter(id => id !== Number(event_id));
                targetEventIds = targetEventIds.concat(ids);
                jointCount = ids.length;
            }
        } catch (e) { /* joint_group 테이블 없음 → 단일 종목만 */ }
    }

    // 되돌리기용 스냅샷 (기록·시기·종합 점수, 종목 상태) — 24시간 안에 되살릴 수 있다 (lib/undo.js)
    let undoId = null;
    try {
        const ph = targetEventIds.map(() => '?').join(',');
        const gL0 = evt.gender === 'M' ? '남자' : evt.gender === 'F' ? '여자' : '혼성';
        const snap = await require('../undo').snapshot(db, {
            competition_id: evt.competition_id, kind: 'results_reset', role: 'operation',
            performed_by: (deps.getJudgeName && deps.getJudgeName(req.headers['x-admin-key'] || req.body.admin_key || '')) || '',
            label: `기록 전체 초기화: ${gL0} ${evt.name}${evt.division ? ' ' + evt.division : ''}`, meta: { event_id: Number(event_id), round_status: evt.round_status },
            tables: [
                { table: 'result', where: `heat_id IN (SELECT id FROM heat WHERE event_id IN (${ph}))`, params: targetEventIds },
                { table: 'height_attempt', where: `heat_id IN (SELECT id FROM heat WHERE event_id IN (${ph}))`, params: targetEventIds },
                ...(evt.parent_event_id ? [{ table: 'combined_score', where: 'event_entry_id IN (SELECT id FROM event_entry WHERE event_id=?)', params: [evt.parent_event_id] }] : []),
            ],
        });
        undoId = snap ? snap.id : null;
    } catch (e) { console.error('[undo] results snapshot:', e.message); }

    let deletedResults = 0, deletedAttempts = 0;
    for (const eid of targetEventIds) {
        const heats = await db.all('SELECT id FROM heat WHERE event_id=?', eid);
        ...
        if (eid !== Number(event_id)) {
            const me = await db.get('SELECT * FROM event WHERE id=?', eid);
            if (me && (me.round_status === 'in_progress' || me.round_status === 'completed')) {
                await db.run("UPDATE event SET round_status='heats_generated' WHERE id=?", eid);
                broadcastSSE('event_status_changed', { event_id: eid, round_status: 'heats_generated' });
            }
            broadcastSSE('result_update', { event_id: eid });
        }
    }
    ...
    opLog(`${evt.name} ${gL} 기록 전체 초기화 (결과 ${deletedResults}건, 시기 ${deletedAttempts}건 삭제${jointCount ? `, 합동 멤버 ${jointCount}종목 포함` : ''})`, 'record', 'system', evt.competition_id);
    res.json({ success: true, deletedResults, deletedAttempts, jointEvents: jointCount, undo_id: undoId });
});

// ============================================================
// HEAT WIND (track events: per-heat wind)

// 조 풍속이 (나중에) 입력·수정되면 그 조 선수들의 신기록 판정을 다시 한다.
//   트랙은 기록이 먼저 들어오고 풍속이 뒤에 오는 일이 흔하다 → 추풍(+2.0 초과)이면 대기 중인 감지를 지우고,
//   허용 풍속으로 고쳐지면 다시 감지한다. 승인/거부가 끝난 로그는 건드리지 않는다.
async function reevaluateHeatRecords(heatId) {
    const heat = await db.get('SELECT * FROM heat WHERE id=?', heatId);
    if (!heat) return { removed: 0 };
    const del = await db.run(
        `DELETE FROM record_breaking_log WHERE status='pending' AND event_id=?
         AND event_entry_id IN (SELECT event_entry_id FROM heat_entry WHERE heat_id=?)`,
        heat.event_id, heatId);
    const rows = await db.all("SELECT * FROM result WHERE heat_id=? AND time_seconds IS NOT NULL AND time_seconds>0 AND (status_code IS NULL OR status_code='')", heatId);
    for (const r of rows) await _runRecordCompareHook(r, heat);
    return { removed: (del && del.changes) || 0, rechecked: rows.length };
}

// 외부 모듈(필드 기록카드 가져오기 등)이 같은 신기록 감지 경로를 재사용할 수 있도록 노출
return { runRecordCompareHook: _runRecordCompareHook, reevaluateHeatRecords };
};
