/**
 * lib/routes/sms.js — SMS System API 모듈
 *
 * 추출 시점: 2026-05-31 (A-11 모듈 추출 1단계)
 * 출처: server.js 라인 11074~11308 (235줄)
 *
 * 6 routes:
 *   GET    /api/admin/sms/config       — 설정 조회 (api_key 마스킹)
 *   POST   /api/admin/sms/config       — 설정 저장
 *   POST   /api/admin/sms/preview      — 메시지 길이/종류 미리보기
 *   POST   /api/admin/sms/send         — 단건 발송
 *   POST   /api/admin/sms/batch-send   — 대량 발송 (상장 모드 등)
 *   GET    /api/admin/sms/log          — 발송 이력 조회
 *
 * 모든 라우트는 admin_key 필수 (req.body.admin_key 또는 req.query.admin_key)
 *
 * 의존성 주입 (server.js 에서 mount 시):
 *   - db                       : DB 어댑터
 *   - isAdminKey               : 관리자 키 검증 함수
 *   - SMS                      : ./lib/smsSender 모듈
 *   - getEventResultsForCert   : 종목별 상위/완주자 결과 조회 헬퍼
 *                                (현재 server.js 의 함수, certificate 모듈 추출 시 함께 이동 예정)
 */
const { buildCertToken } = require('../certLink');

const _batchRunning = new Set();      // 일괄 발송이 진행 중인 대회 id

module.exports = function mountSmsRoutes(app, deps) {
    const { db, isAdminKey, SMS, getEventResultsForCert } = deps;
    if (!app) throw new Error('[lib/routes/sms] app required');
    if (!db) throw new Error('[lib/routes/sms] db required');
    if (!isAdminKey) throw new Error('[lib/routes/sms] isAdminKey required');
    if (!SMS) throw new Error('[lib/routes/sms] SMS module required');
    if (!getEventResultsForCert) throw new Error('[lib/routes/sms] getEventResultsForCert required');

    // 월 카운터 리셋 헬퍼 (async) — 내부 전용
    // 월 발송 한도 — 설정 화면에 있었지만 어디서도 검사하지 않았다(2026-09). 실제 발송(시뮬레이션 제외)만 센다.
    function _quotaExceeded(cfg, willSend) {
        const q = parseInt(cfg.monthly_quota || 0, 10);
        if (!q) return false;                                   // 0 = 한도 없음
        const real = !cfg.sim_mode && !!cfg.api_key;
        return real && (parseInt(cfg.sent_this_month || 0, 10) + (willSend || 1)) > q;
    }
    async function _resetSmsCounterIfNeeded(cfg) {
        const now = new Date();
        const ym = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}`;
        if (cfg.last_reset_month !== ym) {
            await db.run('UPDATE sms_config SET sent_this_month=0, last_reset_month=? WHERE id=1', ym);
            cfg.sent_this_month = 0;
            cfg.last_reset_month = ym;
        }
        return cfg;
    }

    // SMS 설정 조회
    app.get('/api/admin/sms/config', async (req, res) => {
        try {
            const adminKey = req.query.admin_key;
            if (!isAdminKey(adminKey)) return res.status(403).json({ error: '관리자 권한이 필요합니다.' });
            const cfg = await db.get('SELECT * FROM sms_config WHERE id=1');
            if (!cfg) return res.status(500).json({ error: 'sms_config가 초기화되지 않았습니다.' });
            await _resetSmsCounterIfNeeded(cfg);
            // api_key는 마스킹
            const masked = Object.assign({}, cfg, {
                api_key: cfg.api_key ? '***' + cfg.api_key.slice(-4) : '',
                api_key_set: !!cfg.api_key,
            });
            res.json({ config: masked });
        } catch (err) {
            res.status(500).json({ error: err.message });
        }
    });

    // SMS 설정 저장
    app.post('/api/admin/sms/config', async (req, res) => {
        try {
            const { admin_key, provider, api_key, user_id, sender_number, sender_name, sim_mode, default_template, monthly_quota } = req.body || {};
            if (!isAdminKey(admin_key)) return res.status(403).json({ error: '관리자 권한이 필요합니다.' });
            const cur = await db.get('SELECT * FROM sms_config WHERE id=1');
            const now = new Date().toISOString();
            // api_key가 빈 문자열이면 기존 유지 (편의)
            const finalApiKey = (api_key === undefined || api_key === '***UNCHANGED***') ? (cur ? cur.api_key : '') : (api_key || '');
            await db.run(`UPDATE sms_config SET
                provider=?, api_key=?, user_id=?, sender_number=?, sender_name=?,
                sim_mode=?, default_template=?, monthly_quota=?, updated_at=?
                WHERE id=1`,
                provider || (cur && cur.provider) || 'aligo',
                finalApiKey,
                user_id || '',
                SMS.normalizePhone(sender_number || ''),
                sender_name || '',
                sim_mode ? 1 : 0,
                default_template || (cur && cur.default_template) || '',
                parseInt(monthly_quota || 0, 10),
                now
            );
            res.json({ success: true });
        } catch (err) {
            console.error('[SMS][config]', err);
            res.status(500).json({ error: err.message });
        }
    });

    // 메시지 길이/종류 미리보기
    app.post('/api/admin/sms/preview', (req, res) => {
        try {
            const { admin_key, message } = req.body || {};
            if (!isAdminKey(admin_key)) return res.status(403).json({ error: '관리자 권한이 필요합니다.' });
            const bytes = SMS.getMessageBytes(message || '');
            const msgType = SMS.detectMessageType(message || '');
            const cost = msgType === 'LMS' ? 35 : 13;
            res.json({ bytes, msg_type: msgType, est_cost_per_msg: cost });
        } catch (err) {
            res.status(500).json({ error: err.message });
        }
    });

    // 단건 발송
    app.post('/api/admin/sms/send', async (req, res) => {
        try {
            const { admin_key, phone, message, title, competition_id, athlete_id, triggered_by, force_real } = req.body || {};
            if (!isAdminKey(admin_key)) return res.status(403).json({ error: '관리자 권한이 필요합니다.' });
            const cfg = await db.get('SELECT * FROM sms_config WHERE id=1');
            if (!cfg) return res.status(500).json({ error: 'sms_config가 초기화되지 않았습니다.' });
            await _resetSmsCounterIfNeeded(cfg);

            // force_real: 전역 시뮬레이션 모드여도 이 1건만 실제 발송(테스트용).
            //   단, API Key·발신번호가 저장돼 있어야 실제 발송 가능 → 없으면 명확히 안내.
            let sendCfg = cfg;
            if (force_real) {
                if (!cfg.api_key) return res.status(400).json({ error: '실제 발송하려면 알리고 API Key를 먼저 입력·저장하세요. (현재 미설정)' });
                if (!cfg.sender_number) return res.status(400).json({ error: '실제 발송하려면 알리고에 사전 등록한 발신번호를 먼저 입력·저장하세요.' });
                sendCfg = Object.assign({}, cfg, { sim_mode: 0 });
            }

            if (!SMS.isValidMobile(phone)) return res.status(400).json({ error: `휴대폰 번호 형식이 아닙니다: ${phone}` });
            if (_quotaExceeded(sendCfg, 1)) return res.status(429).json({ error: `이번 달 문자 한도(${cfg.monthly_quota}건)를 넘습니다. 현재 ${cfg.sent_this_month}건 발송.`, quota_exceeded: true });
            const result = await SMS.sendOne(sendCfg, { phone, message, title });

            // 로그 기록
            await db.run(`INSERT INTO sms_log
                (competition_id, athlete_id, phone_number, message, status, provider, provider_msg_id, error_message, cost, sent_at, triggered_by)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
                competition_id || null,
                athlete_id || null,
                SMS.normalizePhone(phone),
                message,
                result.status,
                cfg.provider,
                result.provider_msg_id,
                result.error_message,
                result.cost,
                new Date().toISOString(),
                triggered_by || 'manual'
            );

            if (result.status === 'sent' || result.status === 'simulated') {
                await db.run('UPDATE sms_config SET sent_this_month = sent_this_month + 1 WHERE id=1');
            }

            res.json({ success: true, ...result });
        } catch (err) {
            console.error('[SMS][send]', err);
            res.status(500).json({ error: err.message });
        }
    });

    // 일괄 발송 (대회+종목+상장 모드 등 → 자동 메시지 생성)
    app.post('/api/admin/sms/batch-send', async (req, res) => {
        try {
            const { admin_key, competition_id, event_ids, rank_from, rank_to, include_finishers,
                    template, triggered_by, cert_url_base, force_real, rank_scope,
                    heat_number, skip_sent } = req.body || {};
            // rank_scope: 'overall'(종합, 기본) | 'heat'(조별) — 조별이면 순위/필터를 조 내 순위로 계산
            const useHeatRank = rank_scope === 'heat';
            // heat_number: 지정 시 그 조만 발송 (비우면 전체 조). skip_sent: 이미 보낸 (대회·종목·선수) 재발송 방지(기본 ON)
            const heatFilter = (heat_number != null && heat_number !== '') ? parseInt(heat_number, 10) : null;
            const skipSent = skip_sent !== false && skip_sent !== 'false';
            if (!isAdminKey(admin_key)) return res.status(403).json({ error: '관리자 권한이 필요합니다.' });
            if (!competition_id) return res.status(400).json({ error: 'competition_id 필요' });

            const cfg = await db.get('SELECT * FROM sms_config WHERE id=1');
            if (!cfg) return res.status(500).json({ error: 'sms_config가 초기화되지 않았습니다.' });
            await _resetSmsCounterIfNeeded(cfg);

            // force_real: 전역 시뮬레이션 모드여도 이번 일괄만 실제 발송. (단건 발송과 동일 정책)
            //   API Key·발신번호 미설정이면 실발송 불가 → 400으로 명확히 안내.
            let sendCfg = cfg;
            if (force_real) {
                if (!cfg.api_key) return res.status(400).json({ error: '실제 발송하려면 알리고 API Key를 먼저 입력·저장하세요. (현재 미설정)' });
                if (!cfg.sender_number) return res.status(400).json({ error: '실제 발송하려면 알리고에 사전 등록한 발신번호를 먼저 입력·저장하세요.' });
                sendCfg = Object.assign({}, cfg, { sim_mode: 0 });
            }

            const comp = await db.get('SELECT * FROM competition WHERE id=?', competition_id);
            if (!comp) return res.status(404).json({ error: '대회를 찾을 수 없습니다.' });

            let targetEventIds = Array.isArray(event_ids) ? event_ids.slice() : [];
            if (targetEventIds.length === 0) {
                const all = await db.all(`SELECT id FROM event WHERE competition_id=? AND round_type='final' ORDER BY sort_order, id`, competition_id);
                targetEventIds = all.map(e => e.id);
            }

            const rankFrom = Math.max(1, parseInt(rank_from || 1, 10));
            const rankTo = Math.max(rankFrom, parseInt(rank_to || 3, 10));
            const wantFinishers = !!include_finishers;
            const tplMsg = template || cfg.default_template;

            // 상장 링크 기준 URL: cert_url_base(override) > 환경변수 > 요청 호스트 순.
            //   최종 링크는 선수별 서명 토큰으로 `${base}/c/${token}` 형태. (공개 라우트 GET /c/:token)
            const _rawBase = cert_url_base || process.env.PUBLIC_BASE_URL
                || (req.get && req.get('host') ? `https://${req.get('host')}` : '');
            const baseUrl = String(_rawBase).replace(/\/+$/, '');

            // 중복 방지: 이미 발송된 (종목|선수) 조합 미리 로드
            //   이번이 '실제 발송'이면 실제로 나간 건(sent)만 본다 — 예전엔 시뮬레이션 기록(simulated)도 '보냄'으로 쳐서,
            //   리허설을 한 번 돌리고 나면 실제 발송이 전원 건너뛰어졌다(total 0). 시뮬레이션 실행일 때는 둘 다 건너뛴다.
            const _realRun = !sendCfg.sim_mode && !!sendCfg.api_key;
            const _dedupeStatuses = _realRun ? "('sent')" : "('sent','simulated')";
            const sentKeys = new Set();
            if (skipSent) {
                const sentRows = await db.all(
                    `SELECT event_id, athlete_id FROM sms_log WHERE competition_id=? AND status IN ${_dedupeStatuses}`,
                    competition_id
                );
                sentRows.forEach(s => sentKeys.add(`${s.event_id}|${s.athlete_id}`));
            }

            const recipients = [];
            let skippedSent = 0;
            for (const eid of targetEventIds) {
                const { event, rows } = await getEventResultsForCert(eid);
                if (!event) continue;
                for (const row of rows) {
                    // 조 지정 발송이면 해당 조만
                    if (heatFilter != null && row.heat_number !== heatFilter) continue;
                    // 선수 폰번호 조회 (athlete table에 phone 컬럼이 있어야 함 — 없으면 빈 칸으로 시뮬레이션)
                    // 조별 발송이면 순위 기준을 조 내 순위(heat_rank)로 전환
                    const scopeRank = useHeatRank ? row.heat_rank : row.rank;
                    let include = false;
                    // 미출전(기록 없음) 제외 — 순위 범위 발송은 실제 완주한 선수만 대상
                    if (scopeRank != null && scopeRank >= rankFrom && scopeRank <= rankTo && row.finished) include = true;
                    if (!include && wantFinishers && row.finished) include = true;
                    if (!include) continue;
                    // 이미 보낸 (종목|선수) 는 스킵
                    if (skipSent && sentKeys.has(`${eid}|${row.athlete_id}`)) { skippedSent++; continue; }

                    const ath = await db.get('SELECT * FROM athlete WHERE id=?', row.athlete_id);
                    const phone = ath && (ath.phone || ath.phone_number || '') || '';
                    let certUrl = '';
                    if (baseUrl) {
                        const token = await buildCertToken(db, { competitionId: competition_id, eventId: eid, athleteId: row.athlete_id });
                        certUrl = `${baseUrl}/c/${token}`;
                    }
                    // 조별이면 우승/준우승 대신 'N위' 로 표기 (조 내 1위 ≠ 종목 우승)
                    const rankLabel = scopeRank == null ? '완주'
                        : useHeatRank ? `${scopeRank}위`
                        : (scopeRank === 1 ? '우승' : scopeRank === 2 ? '준우승' : `${scopeRank}위`);
                    const message = SMS.fillMessageTemplate(tplMsg, {
                        athlete_name: row.athlete_name,
                        team: row.team,
                        event_name: event.name,
                        rank: scopeRank == null ? '' : scopeRank,
                        rank_label: rankLabel,
                        heat: row.heat_number == null ? '' : row.heat_number,
                        heat_label: row.heat_number == null ? '' : `${row.heat_number}조`,
                        heat_rank: row.heat_rank == null ? '' : row.heat_rank,
                        record_value: row.record_value,
                        competition_name: comp.name,
                        cert_url: certUrl,
                    });
                    recipients.push({ athlete_id: row.athlete_id, event_id: eid, heat_number: (row.heat_number == null ? null : row.heat_number), athlete_name: row.athlete_name, phone, message });
                }
            }

            // 같은 대회의 일괄 발송은 한 번에 하나만 — 수백 명이면 프록시 시간 제한을 넘겨 화면에는 오류가 뜨는데 서버는 계속 보내고 있다.
            //   그때 다시 누르면 두 루프가 남은 선수에게 각각 보내 문자가 두 번 나갔다.
            // 번호 형식이 아닌 수신자는 뺀다 (예전엔 그대로 발송을 시도해 실패 로그만 쌓였다)
            const _badPhones = recipients.filter(r => !SMS.isValidMobile(r.phone));
            if (_badPhones.length) { for (const b of _badPhones) recipients.splice(recipients.indexOf(b), 1); }
            if (_quotaExceeded(sendCfg, recipients.length)) {
                return res.status(429).json({ error: `이번 달 문자 한도(${cfg.monthly_quota}건)를 넘습니다. 현재 ${cfg.sent_this_month}건 발송, 이번 대상 ${recipients.length}건.`, quota_exceeded: true, remaining: Math.max(0, parseInt(cfg.monthly_quota, 10) - parseInt(cfg.sent_this_month || 0, 10)) });
            }
            if (_batchRunning.has(String(competition_id))) {
                return res.status(409).json({ error: '이 대회의 일괄 발송이 아직 진행 중입니다. 발송 내역에서 진행 상황을 확인한 뒤 다시 시도하세요.', in_progress: true });
            }
            _batchRunning.add(String(competition_id));
            // 발송 (순차 — 알리고 동시연결 제한 회피)
            const results = [];
            try {
            for (const r of recipients) {
                // 보내기 직전에 한 번 더 확인 (다른 경로의 단건 발송·재시작 뒤 재시도와 겹치는 경우)
                if (skipSent) {
                    const dup = await db.get(`SELECT id FROM sms_log WHERE competition_id=? AND event_id=? AND athlete_id=? AND status IN ${_dedupeStatuses} LIMIT 1`, competition_id, r.event_id, r.athlete_id);
                    if (dup) { skippedSent++; continue; }
                }
                const sendResult = await SMS.sendOne(sendCfg, { phone: r.phone, message: r.message });
                await db.run(`INSERT INTO sms_log
                    (competition_id, athlete_id, event_id, heat_number, phone_number, message, status, provider, provider_msg_id, error_message, cost, sent_at, triggered_by)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
                    competition_id,
                    r.athlete_id,
                    r.event_id,
                    r.heat_number,
                    SMS.normalizePhone(r.phone),
                    r.message,
                    sendResult.status,
                    cfg.provider,
                    sendResult.provider_msg_id,
                    sendResult.error_message,
                    sendResult.cost,
                    new Date().toISOString(),
                    triggered_by || 'cert_batch'
                );
                if (sendResult.status === 'sent' || sendResult.status === 'simulated') {
                    await db.run('UPDATE sms_config SET sent_this_month = sent_this_month + 1 WHERE id=1');
                }
                results.push({
                    athlete_id: r.athlete_id,
                    athlete_name: r.athlete_name,
                    phone: r.phone,
                    status: sendResult.status,
                    error: sendResult.error_message,
                });
            }
            } finally { _batchRunning.delete(String(competition_id)); }

            const summary = results.reduce((acc, r) => {
                acc[r.status] = (acc[r.status] || 0) + 1;
                return acc;
            }, {});

            res.json({ success: true, sim_mode: !!sendCfg.sim_mode, total: results.length, skipped_already_sent: skippedSent, invalid_phone: _badPhones.map(b => ({ athlete_name: b.athlete_name, phone: b.phone })), summary, results });
        } catch (err) {
            console.error('[SMS][batch-send]', err);
            res.status(500).json({ error: err.message });
        }
    });

    // 발송 현황 요약 — 종목·조 단위로 이미 발송(sent/simulated)된 건수
    //   프론트 발송 UI에서 '발송완료' 배지 표시용 (제한 아님, 표시만)
    app.get('/api/admin/sms/sent-summary', async (req, res) => {
        try {
            if (!isAdminKey(req.query.admin_key)) return res.status(403).json({ error: '관리자 권한이 필요합니다.' });
            const compId = req.query.competition_id;
            if (!compId) return res.status(400).json({ error: 'competition_id 필요' });
            const rows = await db.all(
                `SELECT event_id, heat_number, COUNT(*) AS cnt
                 FROM sms_log
                 WHERE competition_id=? AND status IN ('sent','simulated')
                 GROUP BY event_id, heat_number`,
                compId
            );
            res.json({ summary: rows });
        } catch (err) {
            res.status(500).json({ error: err.message });
        }
    });

    // 발송 이력
    app.get('/api/admin/sms/log', async (req, res) => {
        try {
            const adminKey = req.query.admin_key;
            if (!isAdminKey(adminKey)) return res.status(403).json({ error: '관리자 권한이 필요합니다.' });
            const compId = req.query.competition_id;
            const limit = Math.min(parseInt(req.query.limit || 200, 10), 1000);
            const where = compId ? 'WHERE l.competition_id=?' : '';
            const params = compId ? [compId, limit] : [limit];
            const rows = await db.all(`
                SELECT l.*, a.name AS athlete_name, a.team
                FROM sms_log l
                LEFT JOIN athlete a ON a.id = l.athlete_id
                ${where}
                ORDER BY l.sent_at DESC
                LIMIT ?
            `, ...params);
            res.json({ logs: rows });
        } catch (err) {
            res.status(500).json({ error: err.message });
        }
    });
};
