    ...
    app.post('/api/admin/sms/batch-send', async (req, res) => {
        try {
            const { admin_key, competition_id, event_ids, rank_from, rank_to, include_finishers,
                    template, triggered_by, cert_url_base, force_real, rank_scope } = req.body || {};
            ...
            for (const eid of targetEventIds) {
                ...
                for (const row of rows) {
                    ...
                    let include = false;
                    if (scopeRank != null && scopeRank >= rankFrom && scopeRank <= rankTo) include = true;
            ...
            res.json({ success: true, sim_mode: !!sendCfg.sim_mode, total: results.length, summary, results });
        } catch (err) {
