...
function reqMeta(req) {
    // X-Forwarded-For 를 직접 읽으면 클라이언트가 감사로그 IP 를 조작할 수 있다 → app 의 trust proxy 설정을 따르는 req.ip 만 사용
...
module.exports = function mountAuthRoutes(app, deps) {
    ...
    app.post('/api/auth/login', authLimiter, async (req, res) => {
        ...
        try {
            ...
            if (!user) {
                // 존재하지 않는 계정도 bcrypt 한 번 돌려 응답 시간으로 계정 존재 여부를 추측하지 못하게
                try { bcrypt.compareSync(String(password), '$2a$10$CwTycUXWue0Thq9StjUM0uJ8G1oO7c0e6b7yqzj0h0oQ1dY0t0G5a'); } catch (_) {}
            ...
            return res.json({
                ...
                expires_in: 3600,
                // 약한 비밀번호(8자 미만·흔한 값)로 로그인한 경우 — 클라이언트가 변경 화면으로 안내
                weak_password: (() => { try { return String(password).length < 8 || require('../securityCheck')._WEAK.has(String(password).toLowerCase()); } catch (_) { return false; } })(),
            });
        } catch (e) {
    ...
    app.post('/api/auth/change-password', authLimiter, async (req, res) => {
        ...
        if (String(next).length < 8) return res.status(400).json({ error: '새 비밀번호는 8자 이상이어야 합니다.' });
        try { if (require('../securityCheck')._WEAK.has(String(next).toLowerCase())) return res.status(400).json({ error: '너무 흔한 비밀번호입니다. 다른 값을 사용하세요.' }); } catch (_) {}
        try {
