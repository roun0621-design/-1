...
module.exports = function mountReadinessRoutes(app, deps) {
    ...
    app.get('/api/admin/competitions/:id/readiness', async (req, res) => {
        try {
            ...
            // ── 1b. 중·고연맹 요강 (연맹이 KJAF 이거나 초·중·고 부 종목이 있을 때만) ──
            try {
                const kjaf = require('../kjafRules');
                const evRows = await db.all('SELECT e.*, (SELECT COUNT(*) FROM heat h WHERE h.event_id=e.id) AS heat_count FROM event e WHERE e.competition_id=? AND e.parent_event_id IS NULL', comp.id);
                if (kjaf.applies(comp, evRows)) {
                    const enRows = await db.all(`SELECT ee.id, ee.event_id, ee.athlete_id, ee.status, a.name AS athlete_name, a.team, a.bib_number, a.gender
                                                 FROM event_entry ee JOIN event e ON e.id=ee.event_id JOIN athlete a ON a.id=ee.athlete_id WHERE e.competition_id=?`, comp.id);
                    const mRows = await db.all(`SELECT rm.event_entry_id, rm.athlete_id, a.name AS athlete_name, a.gender FROM relay_member rm JOIN athlete a ON a.id=rm.athlete_id WHERE a.competition_id=?`, comp.id);
                    const issues = kjaf.checkEntries({ events: evRows, entries: enRows, members: mRows });
                    const items = issues.map(x => item(x.key, x.label, x.level, `${x.count}건: ${x.detail}`, { tab: 'events', label: '종목 관리' }));
                    if (!items.length) items.push(item('kjaf_ok', '참가 제한·구성', 'ok', '1인 2종목·계주 학교당 1팀·학년부 중복·믹스릴레이 구성 문제 없음'));
                    groups.push({ key: 'kjaf', title: '중·고연맹 요강', items });
                }
            } catch (e) { console.error('[readiness] kjaf rules:', e.message); }
