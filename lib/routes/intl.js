...
const placing = require('../intl/placing');
const { placesInHeats: _placesInHeats } = placing;
...
module.exports = function mountIntlRoutes(app, deps) {
        ...
        try {
            ...
            for (const x of entries) {
                ...
                const ev = { event_id: x.event_id, event_name: x.event_name, gender: x.event_gender, round_type: x.round_type, round_status: x.round_status, category: x.category, parent_event_id: x.parent_event_id,
                    ...
                    result: r ? (() => {
                        const mark = r.status_code ? null : (r.time_seconds != null ? r.time_seconds : r.distance_meters); const tag = placing.recordTag(r.remark); const toks = String(r.remark || '').split(/\s+/);
                        return { time_seconds: r.time_seconds, distance_meters: r.distance_meters, status_code: r.status_code || null, wind: (r.wind != null && r.wind !== '' && isFinite(parseFloat(r.wind))) ? parseFloat(r.wind) : null,
                            place: placeOf.get(r.heat_id + ':' + x.entry_id) || null, heat_count: heatCount.get(Number(x.event_id)) || 0, qual: (String(r.remark || '').match(/\b(Q|q)\b/) || [null])[0], tag,
                            pb_improved: toks.includes('PB') || better({ category: x.category }, mark, x.personal_best), sb_improved: toks.includes('PB') || toks.includes('SB') || better({ category: x.category }, mark, x.season_best) };   // 동기화가 비고에 PB/SB 를 적어 두면(공식 결과로 PB 가 이미 갱신돼 값이 같아도) 경신로 친다
                    })() : null,
            ...
            for (const a of out) for (const ev of a.events) {
                // 7종·10종 세부종목(parent_event_id)은 종목이 아니다 — 100m 조 2위가 은메달로 잡히던 것 (2026-09-26)
                if (ev.relay || ev.parent_event_id || ev.round_type !== 'final' || ev.round_status !== 'completed' || !ev.result || !ev.result.place || ev.result.place > 3 || seenEv.has(ev.event_id)) continue;
    ...
    app.get('/api/events/:id/spotlight', async (req, res) => {
        try {
            ...
            // PG: GROUP BY 에 없는 열은 집계로 (SQLite 는 관대하지만 PG 는 오류)
            const allRes = await db.all(`SELECT r.heat_id, r.event_entry_id, MIN(r.time_seconds) t, MAX(r.distance_meters) d, MAX(COALESCE(r.status_code,'')) sc, MAX(COALESCE(r.remark,'')) rk, MAX(a.name) name, MAX(a.team) team, MAX(a.name_alt) name_alt, MAX(a.barcode) barcode, MAX(h.heat_number) heat_number
            ...
            res.json({ spot: src.spotlight, status: st ? { kind: st.kind, label: st.label, next: st.next || null } : null, rows, pending: pending.map(p => ({ name: p.name, name_alt: p.name_alt, is_team: /^RELAY_/.test(String(p.barcode || '')), heat_number: p.heat_number, lane: p.lane_number, scheduled_at: p.scheduled_at })),
                rule: rule ? { text: rule, text_ko: P.qualRuleKo(rule) } : null, qualified, round_type: ev.parent_event_id ? 'sub' : ev.round_type, round_status: ev.round_status, higher, is_sub: !!ev.parent_event_id });
