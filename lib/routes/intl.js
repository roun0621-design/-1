...
module.exports = function mountIntlRoutes(app, deps) {
    ...
    app.get('/api/competitions/:compId/roster', async (req, res) => {
        ...
        try {
            ...
            const entries = await db.all(`SELECT ee.id AS entry_id, ee.athlete_id, ee.personal_best, ee.season_best,
                ...
                FROM event_entry ee JOIN event e ON e.id=ee.event_id WHERE ee.athlete_id IN (${ph})`, ...ids);
            ...
            const resOf = new Map();
            for (const r of await db.all(`SELECT event_entry_id, time_seconds, distance_meters, status_code, wind, attempt_number FROM result WHERE event_entry_id IN (${eph})`, ...entryIds)) {
            ...
            for (const r of await db.all(`SELECT event_entry_id, MAX(bar_height) AS h FROM height_attempt WHERE event_entry_id IN (${eph}) AND result_mark='O' GROUP BY event_entry_id`, ...entryIds)) {
                const k = Number(r.event_entry_id); if (!resOf.has(k)) resOf.set(k, { distance_meters: r.h, time_seconds: null, status_code: null });
            }
            // 계주 멤버
            ...
            for (const x of entries) {
                ...
                const ev = { event_id: x.event_id, event_name: x.event_name, gender: x.event_gender, round_type: x.round_type, round_status: x.round_status, category: x.category, parent_event_id: x.parent_event_id,
                    ...
                    result: r ? { time_seconds: r.time_seconds, distance_meters: r.distance_meters, status_code: r.status_code, wind: r.wind } : null,
            ...
            res.json({ team, athletes: out.filter(a => !a.is_team), teams: out.filter(a => a.is_team) });
