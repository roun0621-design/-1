...
async function _placesInHeats(db, heatIds) {
    ...
    for (const [hid, rows] of byHeat) {
        const field = /^field/.test(cat.get(hid) || '');
...
module.exports = function mountIntlRoutes(app, deps) {
    ...
    app.get('/api/competitions/:compId/roster', async (req, res) => {
        ...
        try {
            ...
            const resOf = new Map();
            for (const r of await db.all(`SELECT r.heat_id, r.event_entry_id, r.time_seconds, r.distance_meters, r.status_code, COALESCE(r.wind, h.wind) AS wind, r.attempt_number FROM result r JOIN heat h ON h.id=r.heat_id WHERE r.event_entry_id IN (${eph})`, ...entryIds)) {   // 풍속은 결과 행에 없으면 조의 풍속
            ...
            for (const x of entries) {
                ...
                const ev = { event_id: x.event_id, event_name: x.event_name, gender: x.event_gender, round_type: x.round_type, round_status: x.round_status, category: x.category, parent_event_id: x.parent_event_id,
                    ...
                    result: r ? { time_seconds: r.time_seconds, distance_meters: r.distance_meters, status_code: r.status_code, wind: r.wind, place: placeOf.get(r.heat_id + ':' + x.entry_id) || null, heat_count: heatCount.get(Number(x.event_id)) || 0 } : null,
