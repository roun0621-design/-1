...
module.exports = function mountRecordBreaksRoutes(app, deps) {
    ...
    app.post('/api/record-breaks/:id/approve', async (req, res) => {
        try {
            ...
            if (rbl.is_tie && existing) {
                // 타이기록: 기록표의 값·보유자는 그대로(같은 기록), 승인만 남긴다 — 기록지에는 CT/DT/KT 로 표기
            } else if (existing) {
                await db.run(
            ...
            opLog(`🏆 기록 승인: ${event_name} ${record_type.toUpperCase()}${rbl.is_tie ? ' 타이' : ''} (${athlete_name} ${new_value})`,
