...
module.exports = function mountEventsReadRoutes(app, deps) {
    ...
    app.get('/api/events', async (req, res) => {
        ...
        if (events.length > 0) {
            ...
            // 종합경기(7종·10종) 부모: 세부종목의 조·레인을 합산 — 국제대회는 세부종목마다 조가 따로 있어 부모만 보면 0 이다
            const parents = events.filter(e => e.category === 'combined' && !e.parent_event_id);
            if (parents.length) {
                const pids = parents.map(e => e.id), pph = pids.map(() => '?').join(',');
                const subs = await db.all(`SELECT e.parent_event_id AS pid, (SELECT COUNT(*) FROM heat h WHERE h.event_id=e.id) AS hc, (SELECT COUNT(he.id) FROM heat h JOIN heat_entry he ON he.heat_id=h.id WHERE h.event_id=e.id) AS hec FROM event e WHERE e.parent_event_id IN (${pph})`, ...pids);
                const agg = new Map();
                subs.forEach(s => { const a = agg.get(Number(s.pid)) || { hc: 0, hec: 0 }; a.hc += Number(s.hc); a.hec += Number(s.hec); agg.set(Number(s.pid), a); });
                parents.forEach(e => { const a = agg.get(Number(e.id)); if (a && !e.heat_count) { e.heat_count = a.hc; e.heat_entry_count = a.hec; } });
            }
            ...
            if (competition_id) {
                try {
                    ...
                    if (src && src.spotlight) {
                        ...
                        //   7종·10종 세부종목은 부모와 앞부분이 같아서 상속하면 안 된다(남자 10종의 원반던지기가 '한국 선수' 필터에 개별 남자 원반던지기를 끌어왔다) — 세부종목은 직접 출전만
                        events.forEach(e => { e.spotlight = spot.has(Number(e.id)) || (!e.parent_event_id && baseOf(e) != null && spotBases.has(baseOf(e))) ? src.spotlight : null; });
                        // 우리 선수 상태 칩: 결승 진출 · 예선 탈락 · 최종 3위 (lib/intl/placing.spotStatus)
                        try {
                            const stMap = await require('../intl/placing').spotStatus(db, competition_id, src.spotlight, events.filter(e => spot.has(Number(e.id))));
                            events.forEach(e => { const s = stMap.get(Number(e.id)); if (s) e.spot_status = { kind: s.kind, label: s.label, short: s.short, count: s.count, places: s.places || null }; });
                        } catch (e) {}
                    }
                } catch (e) {}
            }
