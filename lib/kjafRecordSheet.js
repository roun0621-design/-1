...
const { parseDivision: _parseDivision } = require('./division');
...
// 종목 칸 표기: 부 접미('100m 중1학년부')는 뗀다 — 시트·블록이 이미 부다
const evLabel = ev => stripDivisionSuffix(ev.name);
const normEv = n => stripDivisionSuffix(n).replace(/[,\s]/g, '').replace(/[×X]/g, 'x').replace(/\(mixed\)/i, '(Mixed)');
...
/** 부 라벨 → { level: 'ELEM'|'MID'|'HIGH'|null, grade } — lib/division 의 규칙. 대학·일반 등은 학교급 없음으로 */
function parseDivision(label) {
    const p = _parseDivision(label);
    return { level: ['ELEM', 'MID', 'HIGH'].includes(p.level) ? p.level : null, grade: p.grade };
}
...
const TIE_CODE = { national: 'KT', division: 'DT', competition: 'CT' };
const RECORD_RANK = { KR: 6, DR: 4, CR: 2, KT: 5, DT: 3, CT: 1 };     // 신기록 > 같은 급 타이
...
function drawBlock(ws, startRow, block, ctx) {
    ...
    for (const ev of evs) {
        ...
        lines.forEach((line, li) => {
            const first = li === 0;
            setCell(ws, r, 2, first ? evLabel(ev) : '', { font: F(7), border: { top: first ? dbl : hair, bottom: hair, left: thin, right: thin } });
            for (let k = 0; k < RANKS; k++) {
                const c = 3 + k * 3;
                const x = line[k];
...
async function generateKjafRecordSheet(db, comp, deps) {
    ...
    try {
        ...
        for (const x of recordRows) {
            const code = (x.is_tie ? TIE_CODE : RECORD_CODE)[x.record_type]; if (!code) continue;
    ...
    // ── 신기록현황 ──
    {
        ...
        const tieLabel = { national: '한국타이', division: '부별타이', competition: '대회타이' };
        let r = 6, n = 1;
        for (const x of recordRows) {
            const d = dayOf.get(x.event_id);
            [n++, d ? `${d}일` : '', gl(x), stripDivisionSuffix(x.ev_name), x.athlete_name, x.athlete_team, x.new_value, x.previous_value, (x.is_tie ? tieLabel : label)[x.record_type] || x.record_type]
