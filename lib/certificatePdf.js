...
function renderRecordCard(doc, tpl, data) {
  ...
  // 카드 외곽(얇은 라운드 보더) + 상단 포인트 바
  doc.save();
  doc.lineWidth(1.2).strokeColor(accent).roundedRect(x0, y0, cardW, cardH, 14).stroke();
  doc.fillColor(accent).rect(x0, y0, cardW, 8).fill();
  doc.restore();

  // ----- 로고 (좌/우 상단) -----
  const logoSize = 52, logoTop = 26;
  const lLogo = imageFile(tpl.logo_left_path);
  if (lLogo) { try { doc.image(lLogo, x0 + 22, logoTop, { fit: [logoSize, logoSize] }); } catch (_) {} }
  const rLogo = imageFile(tpl.logo_right_path);
  if (rLogo) { try { doc.image(rLogo, x0 + cardW - 22 - logoSize, logoTop, { fit: [logoSize, logoSize] }); } catch (_) {} }

  // 제목
  doc.font('B').fontSize(38).fillColor('#1a1a1a');
  const title = (tpl.title_text && tpl.title_text !== '상  장') ? tpl.title_text : '기 록 증';
  const tw = doc.widthOfString(title);
  doc.text(title, (A4_W - tw) / 2, 80, { lineBreak: false });

  // 대회명
  if (data.competition_name) {
    doc.font('R').fontSize(13).fillColor('#8a7f6a');
    const cw = doc.widthOfString(data.competition_name);
    doc.text(data.competition_name, (A4_W - cw) / 2, 132, { lineBreak: false });
  }
  // 구분선
  doc.lineWidth(0.8).strokeColor(accent).moveTo(A4_W / 2 - 60, 164).lineTo(A4_W / 2 + 60, 164).stroke();

  // 이름
  const name = data.athlete_name || '';
  doc.font('B').fontSize(28).fillColor('#1a1a1a');
  const nw = doc.widthOfString(name);
  doc.text(name, (A4_W - nw) / 2, 190, { lineBreak: false });
  ...
  if (subParts.length) {
    ...
    doc.font('R').fontSize(13).fillColor('#666');
    const sw = doc.widthOfString(sub);
    doc.text(sub, (A4_W - sw) / 2, 228, { lineBreak: false });
  }
  ...
  doc.font('R').fontSize(14).fillColor('#333');
  doc.text(proveText, 80, 262, { width: A4_W - 160, align: 'center', lineGap: 6 });
  ...
  panelRows.forEach(([label, value], i) => {
    const cy = ry + rowH / 2;
    doc.font('R').fontSize(13).fillColor('#9a8f78');
    doc.text(label, px + 28, cy - 8, { lineBreak: false });
    const isRec = (label === '기록');
    doc.font('B').fontSize(isRec ? 20 : 15).fillColor(isRec ? '#7a3a00' : '#222');
  ...
  // 확인 문구
  doc.font('R').fontSize(13).fillColor('#444');
  const confirmText = '위에 기재된 내용이 틀림없음을 확인합니다.';
  const cfw = doc.widthOfString(confirmText);
  doc.text(confirmText, (A4_W - cfw) / 2, panelTop + panelH + 26, { lineBreak: false });

  // 날짜
  if (tpl.show_date !== 0) {
    const dateStr = data.date || todayKR();
    doc.font('R').fontSize(14).fillColor('#222');
    const dw = doc.widthOfString(dateStr);
    doc.text(dateStr, (A4_W - dw) / 2, A4_H - 196, { lineBreak: false });
  }
  ...
  const signY = A4_H - 156;
  if (org) {
    doc.font('B').fontSize(18).fillColor('#1a1a1a');
    const ow = doc.widthOfString(org);
    doc.text(org, (A4_W - ow) / 2, signY, { lineBreak: false });
  }
  if (sName || sTitle) {
    doc.font('B').fontSize(18).fillColor('#1a1a1a');
    ...
    const ly = signY + 30;
    doc.text(line, (A4_W - lw) / 2, ly, { lineBreak: false });
    const sealPath = imageFile(tpl.seal_image_path);
    if (sealPath) { try { doc.image(sealPath, (A4_W + lw) / 2 + 8, ly - 10, { fit: [44, 44] }); } catch (_) {} }
  }

  // ----- 워터마크 (맨 위에 옅게 — 패널에 가려지지 않게) -----
...
function renderCertificatePage(doc, template, data) {
  ...
  let infoY = doc.y + 14;
  if (vars.event_name) {
    doc.font('B').fontSize(20).fillColor('#1a3a6e');
