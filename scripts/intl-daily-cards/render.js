...
(async () => {
  const S = process.env.DATA || require('path').join(__dirname, 'work');
  const out = process.argv[2]; const files = JSON.parse(fs.readFileSync(S + '/page.json', 'utf8'));
  const b = await puppeteer.launch({ headless: 'new', args: ['--no-sandbox'] });
  const p = await b.newPage(); await p.setViewport({ width: 1080, height: 1350 });
  await p.goto('file://' + S + '/page.html', { waitUntil: 'networkidle0', timeout: 60000 }); await p.evaluate(() => document.fonts.ready); await new Promise(r => setTimeout(r, 500));
